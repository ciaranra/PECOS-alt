...
Guppy → HUGR → Custom LLVM IR → Direct PECOS Runtime
                    ↑                    ↑
            (non-standard format)  (bypasses LlvmEngine)
```

### New Architecture (Standard QIR + LlvmEngine):
```
Guppy → HUGR → Standard QIR → LlvmEngine → QIR Runtime → Results
                    ↑             ↑            ↑
...
**Purpose**: Generate standard QIR format compatible with existing LlvmEngine
...
- Compatible with `examples/llvm/bell.ll` format
...
- Generates `@main()` function (required by LlvmEngine)
- Adds standard QIR prologue with type definitions
- Outputs format compatible with existing `LlvmEngine::new(qir_file)`

**Key Functions**:
- `add_standard_qir_prologue()` - Adds QIR type definitions
- Function renaming to `@main()` for LlvmEngine compatibility
- Standard QIR format validation

### 3. QIR Engine Wrapper (`qir_engine_wrapper.py`)

**Purpose**: Python wrapper around existing `LlvmEngine` for proper execution
...
- Uses `LlvmEngineWrapper` instead of direct PECOS CLI calls
...
- **LlvmEngine**: Proven quantum program execution
...
- **Existing examples work**: `examples/llvm/bell.ll` format
...
- **LlvmEngine**: Already handles QIR loading, compilation, execution
...
3. **Function Entry Point**: Rename functions to `@main()` for LlvmEngine
...
4. **Python integration** using LlvmEngine pipeline
...
### Execution (LlvmEngine):
```python
qir_file = compile_hugr_to_qir(hugr_bytes)  # → Standard QIR
engine = LlvmEngine::new(qir_file)           # → Proven execution
...
```python
{
    ...
    'shots': 1000,
    'execution_engine': 'pecos_llvm_engine'
}
```
...
3. **Uses proven execution pipeline** (LlvmEngine -> QIR Runtime)
