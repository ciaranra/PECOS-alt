    ...
    match value {
        Value::Object(map) => {
            ...
            for (key, val) in map.clone().iter() {
                if let Value::String(s) = val {
                    if s.contains("List") {
                        ...
                        log::debug!("Replaced List with Array in field {}: {} -> {}", key, s, map[key]);
                    }
                }
            }
            ...
            if let Some(Value::String(variant)) = map.get("variant") {
                if variant == "Array" && !map.contains_key("extension") {
                    map.insert("extension".to_string(), 
