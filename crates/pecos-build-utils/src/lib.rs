...
pub fn report_cache_config() {
    // Only report if explicitly requested via environment variable
    if std::env::var("PECOS_VERBOSE_BUILD").is_err() {
        return;
    }

    println!("cargo:warning=Checking C++ compiler cache configuration...");
    ...
    if cc.contains("ccache") || cc.contains("sccache") {
        println!("cargo:warning=Using compiler cache via CC: {cc}");
    } else if cxx.contains("ccache") || cxx.contains("sccache") {
        println!("cargo:warning=Using compiler cache via CXX: {cxx}");
    } else {
        ...
        if let Ok(wrapper) = std::env::var("RUSTC_WRAPPER") {
            if wrapper.contains("sccache") {
                println!(
                    "cargo:warning=Note: RUSTC_WRAPPER=sccache detected. For C++ caching, also set CC='sccache cc' and CXX='sccache c++'"
                );
            } else if wrapper.contains("ccache") {
                println!(
                    "cargo:warning=Note: RUSTC_WRAPPER=ccache detected. For C++ caching, also set CC='ccache cc' and CXX='ccache c++'"
                );
            }
        }
    }
    ...
    if let Ok(num_jobs) = std::env::var("NUM_JOBS") {
        println!("cargo:warning=Using {num_jobs} parallel jobs for C++ compilation");
    }
}
