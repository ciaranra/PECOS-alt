...
fn create_test_llvm_file(dir: &Path, name: &str) -> PathBuf {
...
fn test_concurrent_same_file_compilation() {
    ...
    let llvm_file = Arc::new(create_test_llvm_file(test_dir.path(), "concurrent_same"));
    ...
    for i in 0..3 {
        ...
        let handle = thread::spawn(move || {
            ...
            let result = LlvmLinker::compile(llvm_file.as_ref(), Some(&output_dir));
...
fn test_concurrent_different_files_compilation() {
    ...
    let llvm_files: Vec<_> = (0..4)
        .map(|i| Arc::new(create_test_llvm_file(test_dir.path(), &format!("file_{i}"))))
        .collect();
...
fn test_runtime_library_concurrent_access() {
    ...
    for i in 0..3 {
        ...
        let handle = thread::spawn(move || {
            ...
            let llvm_file = create_test_llvm_file(test_dir.path(), &format!("runtime_test_{i}"));
            ...
            let output_dir = test_dir.path().join(format!("build_{i}"));
            let result = LlvmLinker::compile(&llvm_file, Some(&output_dir));
