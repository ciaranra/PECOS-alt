...
use pecos_llvm_runtime::linker::LlvmLinker;
use serial_test::serial;

/// Create a simple test LLVM IR file
fn create_test_llvm_file(dir: &Path, name: &str, content_suffix: &str) -> PathBuf {
...
#[test]
#[serial]
fn test_llvm_executable_caching() {
    println!("\n=== Testing LLVM executable caching ===");

    let test_dir = tempfile::tempdir().unwrap();
    let output_dir = test_dir.path().join("build");
    let llvm_file = create_test_llvm_file(test_dir.path(), "cache_test", "v1");

    // First compilation
    println!("1. First compilation...");
    let lib1 = LlvmLinker::compile(&llvm_file, Some(&output_dir)).unwrap();
    ...
    let lib2 = LlvmLinker::compile(&llvm_file, Some(&output_dir)).unwrap();
...
#[test]
#[serial]
fn test_llvm_rebuild_on_source_change() {
    println!("\n=== Testing LLVM rebuild on source change ===");

    let test_dir = tempfile::tempdir().unwrap();
    let output_dir = test_dir.path().join("build");
    let llvm_file = create_test_llvm_file(test_dir.path(), "source_change_test", "v1");
...
#[test]
#[serial]
fn test_llvm_rebuild_on_runtime_update() {
    println!("\n=== Testing LLVM rebuild on runtime update ===");

    let test_dir = tempfile::tempdir().unwrap();
    let output_dir = test_dir.path().join("build");
    let llvm_file = create_test_llvm_file(test_dir.path(), "runtime_update_test", "v1");
...
fn test_multiple_llvm_files_independent_caching() {
    ...
    let llvm1 = create_test_llvm_file(test_dir.path(), "file1", "v1");
    let llvm2 = create_test_llvm_file(test_dir.path(), "file2", "v1");

    // Compile both
    println!("1. Compiling two LLVM files...");
    let lib1 = LlvmLinker::compile(&llvm1, Some(&output_dir)).unwrap();
    let lib2 = LlvmLinker::compile(&llvm2, Some(&output_dir)).unwrap();
    ...
    let lib1_new = LlvmLinker::compile(&llvm1, Some(&output_dir)).unwrap();
    let lib2_new = LlvmLinker::compile(&llvm2, Some(&output_dir)).unwrap();
