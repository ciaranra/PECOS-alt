...
impl LlvmRuntimeState {
    ...
    pub fn store_measurement(&mut self, result_id: usize, value: bool) {
        use log::debug;
        debug!("store_measurement: result_id={} value={}", result_id, value);
    ...
    pub fn get_measurement_result(&self, result_id: usize) -> Option<bool> {
        ...
        debug!("get_measurement_result: all results = {:?}", self.measurement_results);
        result
    }
    ...
    pub fn track_tuple_access(&mut self, result_id: usize) {
        use log::debug;
        debug!("track_tuple_access: result_id={}, current list={:?}", result_id, self.tuple_accessed_results);
        debug!("track_tuple_access: measurement_result_ids={:?}", self.measurement_result_ids);
        if !self.tuple_accessed_results.contains(&result_id) {
            self.tuple_accessed_results.push(result_id);
            debug!("track_tuple_access: added result_id={}, new list={:?}", result_id, self.tuple_accessed_results);
        }
    }
    ...
    pub fn update_measurement_results(&mut self, results: &[u32]) {
        ...
        debug!("update_measurement_results: Processing {} values", results.len());
        debug!("update_measurement_results: Raw input: {:?}", results);
        debug!("update_measurement_results: Current measurement_results before update: {:?}", self.measurement_results);
        for i in (0..results.len()).step_by(2) {
            if i + 1 < results.len() {
                ...
                debug!("store_measurement: pair[{}] result_id={} raw_value={} bool_value={}", 
                       i/2, result_id, raw_value, measurement_value);
                self.measurement_results
                    .insert(result_id, measurement_value);
            }
        }
        debug!("update_measurement_results: Final measurement_results: {:?}", self.measurement_results);
        ...
        if !self.tuple_return.is_empty() && !self.tuple_placeholder_mapping.is_empty() {
            debug!("Updating tuple with placeholder mapping: {:?}", self.tuple_placeholder_mapping);
            ...
            for (&tuple_idx, &result_id) in &self.tuple_placeholder_mapping {
                ...
                if let Some(&measurement) = self.measurement_results.get(&result_id) {
                    ...
                    if tuple_idx < updated_tuple.len() {
                        ...
                        debug!("Updating tuple[{}]: old={}, new={} (result_id={}, measurement={})", 
                               tuple_idx, old_value, new_value, result_id, measurement);
                        if old_value != new_value {
                            updates_made = true;
                        }
                    } else {
                        debug!("ERROR: tuple_idx {} out of bounds (tuple len={})", tuple_idx, updated_tuple.len());
                    }
                } else {
                    debug!("WARNING: No measurement found for result_id={} (tuple_idx={})", result_id, tuple_idx);
                    debug!("Available measurements: {:?}", self.measurement_results.keys().collect::<Vec<_>>());
                }
            }
            
            // Always update (for debugging)
            debug!("Final tuple return values: {:?} (updates_made={})", updated_tuple, updates_made);
    ...
    pub fn export_shot(&self) -> Shot {
        ...
        if !self.tuple_return.is_empty() {
            debug!("Exporting shot with tuple return values: {:?}", self.tuple_return);
            shot.data.insert(
                "result".to_string(),
    ...
    pub fn set_tuple_return(&mut self, values: &[i32]) {
        ...
        if !self.tuple_accessed_results.is_empty() {
            debug!("Mapping {} accessed results to tuple positions", self.tuple_accessed_results.len());
            ...
            for (idx, &result_id) in self.tuple_accessed_results.iter().enumerate() {
                if idx < values.len() {
                    ...
                    debug!("Mapped tuple index {} to result_id {} (current value={})", idx, result_id, values[idx]);
                }
            }
        }
        
        debug!("Final tuple_placeholder_mapping: {:?}", self.tuple_placeholder_mapping);
