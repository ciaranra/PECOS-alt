...
use super::state::LlvmRuntimeState;
use log::error;
...
pub struct RuntimeRegistry {
    states: HashMap<u64, Arc<Mutex<LlvmRuntimeState>>>,
}
...
impl RuntimeRegistry {
    ...
    ///
    /// Panics if the registry lock is poisoned
    pub fn register_runtime(state: Arc<Mutex<LlvmRuntimeState>>) -> u64 {
    ...
    ///
    /// Panics if the registry lock is poisoned
    pub fn get_runtime(id: u64) -> Option<Arc<Mutex<LlvmRuntimeState>>> {
    ...
    where
        F: FnOnce(&mut LlvmRuntimeState) -> R,
    {
        ...
        CURRENT_RUNTIME_ID.with(|current| {
            ...
            Self::initialize();
            let new_state = Arc::new(Mutex::new(LlvmRuntimeState::new()));
            ...
            *current.borrow_mut() = Some(id);
            ...
            match new_state.lock() {
                ...
                Err(e) => {
                    error!("LLVM Runtime: Critical error - failed to lock new runtime state: {e}");
                    // Return a default/fallback result instead of None to avoid crashes
                    // This is a last resort to prevent segfaults
                    panic!("LLVM Runtime: Failed to initialize runtime state");
                }
            }
        })
    }
