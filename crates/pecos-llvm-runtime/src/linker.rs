...
//! - **Source changes**: When pecos-llvm-runtime source files are newer than the library
...
//!   - Creates marker file (`~/.cargo/pecos-llvm-runtime/.needs_rebuild`) if rebuild needed
...
impl LlvmLinker {
    ...
    /// # Arguments
    ///
    /// * `llvm_file` - Path to the LLVM IR (.ll) file to compile
    ...
    ///
    /// Returns an error if:
    /// - The LLVM IR file does not exist or is empty
    /// - LLVM tools are not installed or are the wrong version
    /// - Compilation of the LLVM IR file fails
    ...
    pub fn compile<P: AsRef<Path>>(
        ...
        // Compile LLVM IR to object file using LLVM
    ...
    /// Validate that the LLVM IR file exists and is not empty
    fn validate_llvm_file(llvm_file: &Path) -> Result<(), PecosError> {
        let metadata = fs::metadata(llvm_file).map_err(|_| {
            PecosError::Resource(format!("LLVM IR file not found: {}", llvm_file.display()))
        })?;
