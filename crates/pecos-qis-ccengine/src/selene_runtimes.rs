...
/// # Example
/// ```rust
/// use pecos_qis_ccengine::{qis_control_engine, selene_soft_rz_runtime};
/// use pecos_engines::ClassicalControlEngineBuilder;
///
/// # fn main() -> Result<(), Box<dyn std::error::Error>> {
...
fn find_built_selene_runtime(lib_name: &str) -> Result<PathBuf, RuntimeFetchError> {
    ...
    if !runtime_path.exists() {
        return Err(RuntimeFetchError::InvalidPath(format!(
            "Selene runtime {} not found at {}. Run 'cargo build --release' to build it.",
...
/// # Example
/// ```rust
/// use pecos_qis_ccengine::{selene_runtime_auto, qis_control_engine};
/// use pecos_engines::ClassicalControlEngineBuilder;
///
/// # fn main() -> Result<(), Box<dyn std::error::Error>> {
