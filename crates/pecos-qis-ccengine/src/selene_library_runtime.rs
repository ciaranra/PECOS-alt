...
use pecos_qis_runtime_trait::{QisRuntime, RuntimeError, ClassicalState, Shot, Result};
...
impl QisRuntime for QisSeleneLibraryRuntime {
    ...
    fn execute_until_quantum(&mut self) -> Result<Option<Vec<pecos_qis_interface::QuantumOp>>> {
        let handle = self.runtime_handle
            .as_ref()
            .ok_or_else(|| RuntimeError::FfiError("Runtime not initialized".to_string()))?
            .handle;
