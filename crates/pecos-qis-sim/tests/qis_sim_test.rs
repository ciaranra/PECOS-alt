...
fn test_basic_qis_sim() {
    ...
    let shot_vec = qis_engine()
        .program(QisProgram::from_string(SIMPLE_HADAMARD_IR))
        .to_sim()
        .seed(42)
...
fn test_qis_sim_with_noise() {
    ...
    let shot_vec = qis_engine()
        .program(QisProgram::from_string(BELL_STATE_IR))
        .to_sim()
        .seed(42)
        .workers(2)
...
fn test_qis_sim_qubits_exceeded() {
    ...
    let sim = qis_engine()
        .program(QisProgram::from_string(THREE_QUBIT_IR))
        .to_sim()
        .seed(42)
...
fn test_qis_sim_quantum_engines() {
    ...
    let state_vector_shots = qis_engine()
        .program(QisProgram::from_string(SIMPLE_HADAMARD_IR))
        .to_sim()
        .seed(42)
        .qubits(1)
    ...
    let stabilizer_shots = qis_engine()
        .program(QisProgram::from_string(SIMPLE_HADAMARD_IR))
        .to_sim()
        .seed(42)
        .qubits(1)
...
fn test_qis_sim_custom_noise_models() {
    ...
    let shot_vec_biased = qis_engine()
        .program(QisProgram::from_string(BELL_STATE_IR))
        .to_sim()
        .seed(42)
...
fn test_qis_sim_keep_temp_files() {
    ...
    let _sim = qis_engine().program(QisProgram::from_string(SIMPLE_HADAMARD_IR)).to_sim()
        .build()
        .expect("Build should succeed");
