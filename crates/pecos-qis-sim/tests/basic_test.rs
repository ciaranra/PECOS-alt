...
fn test_qis_sim_builder_creation() {
    ...
    let builder = sim_builder()
        .classical(qis_engine().program(QisProgram::from_string("@main() { ret void }")));
...
fn test_multiple_input_formats() {
    ...
    // let _ = sim_builder().classical(qis_engine().program(QisProgram::from_file("test.ll"))); // Would fail

    // Test HUGR bytes
    let _ = sim_builder().classical(qis_engine().program(HugrProgram::from_bytes(vec![1, 2, 3])));

    // Test HUGR file (doesn't need to exist for builder creation - but will fail when actually used)
    // let _ = sim_builder().classical(qis_engine().program(HugrProgram::from_file("test.hugr"))); // Would fail
}
