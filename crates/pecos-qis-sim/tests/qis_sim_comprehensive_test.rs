//! Comprehensive tests for LLVM simulation API matching and exceeding `QisEngine` test coverage
//!
//! These tests ensure that the unified LLVM simulation API provides at least the same functionality as
//! `QisEngine`, plus tests for its additional features like noise models and parallelization.
...
// =============================================================================
// Tests matching QisEngine coverage
// =============================================================================

#[test]
fn test_qis_sim_bell_state_immediate_measurement() {
    if skip_if_llvm_missing("test_qis_sim_bell_state_immediate_measurement") {
        return;
    }

    // Run Bell state with unified API (matches test_bell_state_immediate_measurement)
    let shot_vec = sim_builder()
        .classical(qis_engine().program(QisProgram::from_file(get_bell_path()).unwrap()))
...
#[test]
fn test_qis_sim_qprog_adaptive_algorithm() {
    if skip_if_llvm_missing("test_qis_sim_qprog_adaptive_algorithm") {
        return;
    }

    // Run adaptive algorithm with unified API (matches test_qprog_adaptive_algorithm)
    let shot_vec = sim_builder()
        .classical(qis_engine().program(QisProgram::from_file(get_qprog_path()).unwrap()))
        .seed(42)
        .workers(2)
        .qubits(3)
...
#[test]
fn test_qis_sim_single_worker() {
    if skip_if_llvm_missing("test_qis_sim_single_worker") {
        return;
    }

    // Test with single worker (matches test_qis_bell_state_single_worker)
    let shot_vec = sim_builder()
        .classical(qis_engine().program(QisProgram::from_file(get_bell_path()).unwrap()))
...
#[test]
fn test_qis_sim_with_uniform_depolarizing_noise() {
    if skip_if_llvm_missing("test_qis_sim_with_uniform_depolarizing_noise") {
        return;
    }
...
#[test]
fn test_qis_sim_with_custom_depolarizing_noise() {
    if skip_if_llvm_missing("test_qis_sim_with_custom_depolarizing_noise") {
        return;
    }
...
#[test]
fn test_qis_sim_parallel_execution_scaling() {
    if skip_if_llvm_missing("test_qis_sim_parallel_execution_scaling") {
        return;
    }
...
fn test_qis_sim_quantum_engines() {
    if skip_if_llvm_missing("test_qis_sim_quantum_engines") {
        return;
    }
...
fn test_qis_sim_build_once_run_many() {
    if skip_if_llvm_missing("test_qis_sim_build_once_run_many") {
        return;
    }
...
#[test]
fn test_qis_sim_in_memory_string() {
    if skip_if_llvm_missing("test_qis_sim_in_memory_string") {
        return;
    }
...
#[test]
fn test_qis_sim_reproducibility_with_seed() {
    if skip_if_llvm_missing("test_qis_sim_reproducibility_with_seed") {
        return;
    }
...
fn test_qis_sim_error_handling() {
    if skip_if_llvm_missing("test_qis_sim_error_handling") {
        return;
    }
    ...
    let result = qis_engine()
        .program(QisProgram::from_string(invalid_ir))
        .to_sim()
        .qubits(1)
        .run(10);
    ...
    ";
    let result = qis_engine()
        .program(QisProgram::from_string(no_entry_ir))
        .to_sim()
        .qubits(1)
        .run(10);
    ...
    let result = QisProgram::from_file("/non/existent/file.ll");
    assert!(result.is_err(), "Non-existent file should fail");
}

#[test]
fn test_qis_sim_verbose_options() {
    if skip_if_llvm_missing("test_qis_sim_verbose_options") {
        return;
    }
