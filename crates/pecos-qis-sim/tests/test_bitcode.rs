...
fn test_bitcode_in_memory() {
    ...
    let sim = sim_builder()
        .classical(qis_engine().program(QisProgram::from_bitcode(bitcode)))
...
fn test_bitcode_file() {
    ...
    let sim = sim_builder()
        .classical(qis_engine().program(QisProgram::from_file(&bc_file).unwrap()))
        .build();
