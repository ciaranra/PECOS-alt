...
This crate provides the `qis_engine()` and `qis_sim()` functions that integrate with
...
/// Create a new LLVM simulation builder (thin wrapper around `qis_engine().program().to_sim()`)
///
/// This function creates a `TypedSimBuilder` that uses the unified simulation API.
///
/// # Example
/// ```rust,no_run
/// use pecos_qis_sim::qis_sim;
/// use pecos_programs::QisProgram;
/// use pecos_engines::noise::DepolarizingNoiseModelBuilder;
///
...
/// ```
#[must_use]
pub fn qis_sim(llvm_ir: impl Into<String>) -> pecos_engines::SimBuilder {
    use pecos_engines::ClassicalControlEngineBuilder;
    use pecos_programs::QisProgram;
