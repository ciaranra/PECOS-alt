...
impl QirEngine {
    ...
    fn log_error<E: std::fmt::Display>(context: &str, error: E) -> QueueError {
        let thread_id = get_thread_id();
        warn!("QIR Engine: [Thread {}] {}: {}", thread_id, context, error);
        QueueError::OperationError(format!("{context}: {error}"))
    }
    ...
    pub fn set_assigned_shots(&mut self, shots: usize) -> Result<(), QueueError> {
        debug!(
    ...
    fn setup_library(&mut self) -> Result<(), QueueError> {
        // Get the current thread ID for logging
        let thread_id = get_thread_id();
    ...
    fn process_measurements(&mut self, message: &ByteMessage) -> Result<(), QueueError> {
    ...
    pub fn compile(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let _library_path = QirCompiler::compile(&self.qir_file, None)?;
        debug!("QIR: Compilation successful");
        Ok(())
    }

    /// Pre-compile the QIR library to prepare for cloning
    pub fn pre_compile(&mut self) -> Result<(), QueueError> {
        // Get the current thread ID for logging
        let thread_id = get_thread_id();
    ...
    fn commands_to_byte_message(commands: &[QuantumCommand]) -> Result<ByteMessage, QueueError> {
    ...
    /// # Returns
    ///
    /// * `Result<Vec<QuantumCmd>, QueueError>` - The quantum commands generated by the QIR program
    ///
    /// # Error Handling
    ///
    /// Errors are propagated through the Result type and logged at their source with
    /// appropriate context, including the thread ID.
    fn run_qir_program(&self, library: &QirLibrary) -> Result<Vec<QuantumCmd>, QueueError> {
        ...
        library.call_function(b"main").map_err(|e| {
            ...
            if e.to_string().contains("No such file or directory") {
                ...
                QueueError::OperationError("Library file was already removed".to_string())
            } else {
    ...
    fn generate_commands(&mut self) -> Result<ByteMessage, QueueError> {
        ...
        if let Some(library) = &self.library {
            ...
            Err(QueueError::OperationError(
                "No QIR library loaded".to_string(),
            ))
        }
    }
    ...
    fn find_qubit_allocations(content: &str) -> (usize, bool) {
        ...
        let direct_pattern = Regex::new(r"inttoptr\s*\(\s*i64\s+(\d+)\s+to\s+%Qubit\*\)").unwrap();
        ...
        let alloc_pattern = Regex::new(r"__quantum__rt__qubit_allocate\(\)").unwrap();
    ...
    fn analyze_qir_file(&self) -> Result<usize, QirError> {
        let thread_id = get_thread_id();
        debug!(
        ...
        if !self.qir_file.exists() {
            return Err(error::file_not_found(self.qir_file.clone()));
        }

        // Read the file content
        let content = fs::read_to_string(&self.qir_file)
            .map_err(|e| error::file_read_error(self.qir_file.clone(), e))?;

        // Check if the file is empty
        if content.is_empty() {
            return Err(error::empty_file(self.qir_file.clone()));
        }
        ...
        if found_allocation {
            ...
            Err(error::no_qubit_allocations_found(self.qir_file.clone()))
        }
    }

    /// Helper method to compile the QIR file to a library
    fn compile_library(&self, output_dir: &Path) -> Result<PathBuf, QueueError> {
        let thread_id = get_thread_id();
        ...
        QirCompiler::compile(&self.qir_file, Some(&output_dir_path))
            .map_err(|e| Self::log_error("Failed to compile QIR program", e))
    }
}
    ...
    fn num_qubits(&self) -> usize {
        ...
        match self.analyze_qir_file() {
            ...
            Err(e) => {
                ...
                let message = match &e {
                    QirError::FileNotFound(path) => format!("QIR file not found at {path:?}"),
                    QirError::EmptyFile(path) => format!("QIR file is empty at {path:?}"),
                    QirError::NoQubitAllocationsFound(path) => {
                        format!("No qubit allocations found in QIR file at {path:?}")
                    }
                    _ => format!("{e}"),
                };
