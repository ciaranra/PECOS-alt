...
/// # Note
///
/// This is an internal implementation detail used by `QisEngine`.
/// Users should interact with `QisEngine` instead of using `QisLibrary` directly.
///
/// # Example - Use `QisEngine` instead
///
/// ```no_run
/// use pecos_qis_runtime::prelude::*;
/// use std::path::PathBuf;
///
/// // Create an LLVM engine (this handles library loading internally)
/// let mut engine = QisEngine::new(PathBuf::from("program.ll"));
///
/// // Run a single execution (the engine manages the library for you)
/// let shot = engine.process(()).unwrap();
/// println!("Result: {:?}", shot);
/// ```
pub struct QisLibrary {
...
impl QisLibrary {
    ...
    /// initializes the `QisLibrary` struct for interacting with it.
    ///
    /// # Arguments
    ///
    ...
    pub fn call_function(&self, name: &[u8]) -> Result<i32, PecosError> {
        ...
        unsafe {
            ...
            // Try void signature
                ...
                debug!("  Tried: fn() -> (i32, i32, i32, i32)");
                debug!("  Tried: fn() -> (bool, bool, bool, bool)");
                debug!("  Tried: fn() -> u8");
                debug!("  Tried: fn() -> u32");
                debug!("  Tried: fn() -> u64");
                debug!("  Tried: fn() -> void");
    ...
    /// # Returns
    ///
    /// * `Result<usize, PecosError>` - The number of measurements executed
    ///
    /// # Errors
    ///
    /// This method can return the following errors:
    ...
    /// Gets the measurement result IDs from the runtime
    ///
    /// # Returns
    ///
    /// * `Result<Vec<usize>, PecosError>` - The measurement result IDs in order
    ///
    /// # Errors
    ///
    /// This method can return the following errors:
    ...
    pub fn get_measurement_result_ids(&self) -> Result<Vec<usize>, PecosError> {
        ...
        let result_ids = unsafe {
            let ffi_data = &*ffi_ptr;
            let mut ids = Vec::with_capacity(ffi_data.count);
            for i in 0..ffi_data.count {
                ids.push(*ffi_data.ids.add(i));
            }
            ids
        };
