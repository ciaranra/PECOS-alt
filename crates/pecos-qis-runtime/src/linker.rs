//! QIS Linker Module
//!
//! This module is responsible for compiling QIS (Quantum Instruction Set) programs (.ll files) and linking them
//! with the pre-built runtime library to create dynamically loadable libraries.
//!
//! # Overview
//!
//! The QIS compilation process involves:
//! 1. Compiling the QIS LLVM IR file to an object file using LLVM tools
...
//!
//! ## 1. Static Runtime Library (`~/.cargo/pecos-qis-runtime/libpecos_qis_runtime.a`)
//!
//! The runtime library rebuild is triggered by:
//! - **Missing library**: If the library doesn't exist at all
//! - **Source changes**: When pecos-qis-runtime source files are newer than the library
...
//!   - Creates marker file (`~/.cargo/pecos-qis-runtime/.needs_rebuild`) if rebuild needed
...
//!
//! ## 2. QIS Executables (Compiled QIS LLVM IR linked with runtime)
//!
//! QIS executable rebuild is triggered by:
//! - **Missing executable**: If the compiled library doesn't exist
//! - **QIS source changes**: When the .ll file is newer than the executable
//! - **Runtime library changes**: When the runtime library is newer than the executable
//!
//! The `QisLinker::compile` method handles this by:
//! 1. Checking for cached QIS executable
//! 2. Ensuring runtime library is built/current (via `RuntimeBuilder`)
//! 3. Comparing timestamps: executable vs QIS source and runtime library
...
/// Links QIS programs with the runtime library to create dynamically loadable libraries
...
impl QisLinker {
    ...
    /// # Arguments
    ///
    /// * `llvm_file` - Path to the QIS LLVM IR (.ll) file to compile
    /// * `output_dir` - Optional output directory (defaults to `<llvm_dir>/build/`)
    ///
    /// # Returns
    ///
    ...
    ///
    /// Returns an error if:
    /// - The QIS LLVM IR file does not exist or is empty
    /// - LLVM tools are not installed or are the wrong version
    /// - Compilation of the QIS LLVM IR file fails
    ...
    pub fn compile<P: AsRef<Path>>(
        ...
        let llvm_file = llvm_file.as_ref();
        // Validate the QIS file
        Self::validate_llvm_file(llvm_file)?;

        // Determine output directory
        let output_dir = Self::prepare_output_directory(llvm_file, output_dir)?;

        // Step 1: Check for cached QIS executable
        // We check cache first to avoid updating runtime timestamp unnecessarily
        if let Some(cached_lib) = Self::find_cached_library(llvm_file, &output_dir)? {
        ...
        // Step 4: Build QIS executable
        // Get the runtime library path (already built in steps above)
        let rust_runtime_lib = RuntimeBuilder::build_runtime()?;

        // Generate consistent file paths for caching
        let (object_file, library_file) = Self::generate_file_paths(llvm_file, &output_dir);
        ...
        // Compile QIS IR to object file using LLVM tools
        Self::compile_to_object_file(llvm_file, &object_file)?;
    ...
    fn find_cached_library(
        ...
        let llvm_metadata = fs::metadata(llvm_file)?;
        let llvm_modified = llvm_metadata.modified().map_err(PecosError::IO)?;

        let file_stem = llvm_file
            .file_stem()
            .unwrap_or_else(|| "llvm_program".as_ref())
            .to_string_lossy();
        ...
        if library_file.exists() {
            // Check if library is newer than QIS file
    ...
    /// Validate that the QIS file exists and is not empty
    fn validate_llvm_file(llvm_file: &Path) -> Result<(), PecosError> {
        let metadata = fs::metadata(llvm_file).map_err(|_| {
            PecosError::Resource(format!("QIS file not found: {}", llvm_file.display()))
        })?;
    ...
    fn generate_file_paths(llvm_file: &Path, output_dir: &Path) -> (PathBuf, PathBuf) {
        let file_stem = llvm_file
            .file_stem()
            .unwrap_or_else(|| "llvm_program".as_ref())
            .to_string_lossy();
    ...
    fn compile_to_object_file(llvm_file: &Path, object_file: &Path) -> Result<(), PecosError> {
        ...
        #[cfg(not(target_os = "windows"))]
        {
            ...
            debug!("Successfully compiled LLVM IR to object file");
            Ok(())
        }
    }
    ...
    fn handle_command_status(
        ...
        if !output.status.success() {
            ...
            let error = PecosError::Processing(format!(
                "LLVM IR compilation failed: {command_name} failed with status: {} and error: {stderr}",
                output.status
            ));
            warn!("{error}");
            return Err(error);
