//! QIS Engine Module
//!
//! This module provides the QIS Engine for executing quantum programs compiled to QIS LLVM IR.
use crate::library::QisLibrary;
use crate::linker::QisLinker;
...
impl QisEngine {
    ...
    pub fn new(llvm_file: PathBuf) -> Self {
        ...
        Self {
            ...
            shot_count: 0,
            config: QisEngineConfig::default(),
            entry_point: None,
    ...
    #[must_use]
    pub fn with_config(llvm_file: PathBuf, config: QisEngineConfig) -> Self {
        debug!(
    ...
    fn setup_library(&mut self) -> Result<(), PecosError> {
        ...
        let library = QisLibrary::load(&library_path)
    ...
    pub fn pre_compile(&mut self) -> Result<(), PecosError> {
        ...
        let library_path = QisLinker::compile(&self.llvm_file, None).map_err(|e| {
    ...
    fn run_llvm_program(&self, library: &QisLibrary) -> Result<ByteMessage, PecosError> {
    ...
    fn compile_library(&self, output_dir: &Path) -> Result<PathBuf, PecosError> {
        ...
        QisLinker::compile(&self.llvm_file, Some(&output_dir_path))
...
impl ClassicalEngine for QisEngine {
    ...
    fn compile(&self) -> Result<(), PecosError> {
        ...
        QisLinker::compile(&self.llvm_file, None)
