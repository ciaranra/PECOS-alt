//! Build script for pecos-qis-runtime
//!
...
//!
//! A static library containing all pecos-qis-runtime symbols needed by LLVM IR programs.
...
//! - Checks if the static library exists at `~/.cargo/pecos-qis-runtime/libpecos_qis_runtime.a`
//! - Compares library timestamp against source files in `src/`
//! - Creates marker file (`~/.cargo/pecos-qis-runtime/.needs_rebuild`) if outdated
...
//! 1. Creating a wrapper crate that depends on pecos-qis-runtime
//! 2. Building that crate from within pecos-qis-runtime's build.rs
//! 3. This creates a circular dependency: pecos-qis-runtime -> build.rs -> wrapper -> pecos-qis-runtime
//!
