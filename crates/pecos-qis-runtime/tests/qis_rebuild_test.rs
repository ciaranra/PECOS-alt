//! Tests for LLVM executable rebuild scenarios
//!
//! This test suite verifies that LLVM executables are properly cached and
...
use pecos_qis_runtime::linker::QisLinker;
use serial_test::serial;

/// Create a simple test LLVM IR file
fn create_test_qis_file(dir: &Path, name: &str, content_suffix: &str) -> PathBuf {
    let llvm_file = dir.join(format!("{name}.ll"));
    fs::write(
        &llvm_file,
        format!(
...
#[test]
#[serial]
fn test_qis_executable_caching() {
    println!("\n=== Testing LLVM executable caching ===");

    let test_dir = tempfile::tempdir().unwrap();
    let output_dir = test_dir.path().join("build");
    let llvm_file = create_test_qis_file(test_dir.path(), "cache_test", "v1");

    // First compilation
    println!("1. First compilation...");
    let lib1 = QisLinker::compile(&llvm_file, Some(&output_dir)).unwrap();
    ...
    let lib2 = QisLinker::compile(&llvm_file, Some(&output_dir)).unwrap();
...
#[test]
#[serial]
fn test_qis_rebuild_on_source_change() {
    println!("\n=== Testing LLVM rebuild on source change ===");

    let test_dir = tempfile::tempdir().unwrap();
    let output_dir = test_dir.path().join("build");
    let llvm_file = create_test_qis_file(test_dir.path(), "source_change_test", "v1");
    ...
    // Modify LLVM IR file
    println!("2. Modifying LLVM IR source file...");
    let content = fs::read_to_string(&llvm_file).unwrap();
    fs::write(&llvm_file, content.replace("v1", "v2")).unwrap();
...
#[test]
#[serial]
fn test_qis_rebuild_on_runtime_update() {
    println!("\n=== Testing LLVM rebuild on runtime update ===");

    let test_dir = tempfile::tempdir().unwrap();
    let output_dir = test_dir.path().join("build");
    let llvm_file = create_test_qis_file(test_dir.path(), "runtime_update_test", "v1");
    ...
    // Verify runtime is now newer than the LLVM executable
    ...
    println!("   LLVM executable was rebuilt after runtime update");
}

#[test]
#[serial]
fn test_multiple_llvm_files_independent_caching() {
    println!("\n=== Testing independent caching for multiple LLVM files ===");

    let test_dir = tempfile::tempdir().unwrap();
    let output_dir = test_dir.path().join("build");

    // Create two different LLVM files
    let llvm1 = create_test_qis_file(test_dir.path(), "file1", "v1");
    let llvm2 = create_test_qis_file(test_dir.path(), "file2", "v1");

    // Compile both
    println!("1. Compiling two LLVM files...");
    let lib1 = QisLinker::compile(&llvm1, Some(&output_dir)).unwrap();
    let lib2 = QisLinker::compile(&llvm2, Some(&output_dir)).unwrap();

    assert_ne!(
        lib1, lib2,
        "Different LLVM files should produce different libraries"
    );
    ...
    // Modify only the first LLVM file
    println!("2. Modifying only the first LLVM file...");
    let content = fs::read_to_string(&llvm1).unwrap();
    fs::write(&llvm1, content.replace("v1", "v2")).unwrap();

    // Recompile both
    println!("3. Recompiling both files...");
    let lib1_new = QisLinker::compile(&llvm1, Some(&output_dir)).unwrap();
    let lib2_new = QisLinker::compile(&llvm2, Some(&output_dir)).unwrap();
    ...
    println!("   Only modified LLVM file was rebuilt, other used cache");
}
...
fn get_runtime_lib_path() -> PathBuf {
    ...
    let lib_name = if cfg!(target_os = "windows") {
        "pecos_qis_runtime.lib"
    } else {
        "libpecos_qis_runtime.a"
    };
    base_dir.join("pecos-qis-runtime").join(lib_name)
}
