//! Tests for concurrent LLVM compilation scenarios
//!
//! This test suite verifies that multiple LLVM compilations can happen
...
/// Create a simple test LLVM file
...
fn test_concurrent_same_file_compilation() {
    println!("\n=== Testing concurrent compilation of same LLVM file ===");

    let test_dir = Arc::new(tempfile::tempdir().unwrap());
    let llvm_file = Arc::new(create_test_qis_file(test_dir.path(), "concurrent_same"));
    ...
    for i in 0..3 {
        ...
        let llvm_file = Arc::clone(&llvm_file);

        let handle = thread::spawn(move || {
            println!("Thread {i} starting compilation...");
            let output_dir = test_dir.path().join(format!("build_{i}"));
            let result = QisLinker::compile(llvm_file.as_ref(), Some(&output_dir));
...
fn test_concurrent_different_files_compilation() {
    println!("\n=== Testing concurrent compilation of different LLVM files ===");

    let test_dir = Arc::new(tempfile::tempdir().unwrap());

    // Create multiple LLVM files
    let llvm_files: Vec<_> = (0..4)
        .map(|i| Arc::new(create_test_qis_file(test_dir.path(), &format!("file_{i}"))))
        .collect();

    // Spawn threads to compile different files
    let mut handles = vec![];

    for (i, llvm_file) in llvm_files.into_iter().enumerate() {
        let test_dir = Arc::clone(&test_dir);
...
fn test_runtime_library_concurrent_access() {
    ...
    // at the same time (through QisLinker::compile)
    ...
    for i in 0..3 {
        ...
        let handle = thread::spawn(move || {
            // Create a unique LLVM file for this thread
            let llvm_file = create_test_qis_file(test_dir.path(), &format!("runtime_test_{i}"));
