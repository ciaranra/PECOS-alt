...
fn get_marker_path() -> PathBuf {
    ...
    base_dir.join("pecos-qis-runtime").join(".needs_rebuild")
}
...
/// Helper to create a simple test LLVM file
fn create_test_qis_file(dir: &Path, name: &str) -> PathBuf {
...
fn test_marker_based_rebuild_system() {
    println!("\n=== Testing marker-based rebuild system (via QisLinker) ===");

    let marker_path = get_marker_path();
    let lib_path = get_runtime_lib_path();

    // Create a test LLVM file
    let test_dir = tempfile::tempdir().unwrap();
    let llvm_file = create_test_qis_file(test_dir.path(), "marker_test");
    let output_dir = test_dir.path().to_path_buf();

    // Step 1: Test normal build (no marker)
    test_normal_build(&marker_path, &lib_path, &llvm_file, &output_dir);

    // Step 2: Test with marker file
    test_build_with_marker(&marker_path, &lib_path, &llvm_file, &output_dir, &test_dir);

    // Step 3: Test without changes (should use existing library)
    test_no_rebuild(&lib_path, &llvm_file, &output_dir);

    println!("\n=== All marker-based rebuild tests passed! ===\n");
}

fn test_normal_build(marker_path: &Path, lib_path: &Path, llvm_file: &Path, output_dir: &Path) {
    ...
    // Compile LLVM (this will trigger runtime build if needed)
    let result = QisLinker::compile(&llvm_file, Some(&output_dir));
    assert!(result.is_ok(), "QisLinker::compile() failed: {result:?}");
...
fn test_build_with_marker(
    ...
    // Compile LLVM again (should trigger runtime rebuild due to marker)
    println!("   - Starting LLVM compilation...");
    let result2 = QisLinker::compile(&llvm_file, Some(&output_dir));
    assert!(
        result2.is_ok(),
        "QisLinker::compile() failed with marker: {result2:?}"
    );
    println!("   - LLVM compilation completed");
...
fn test_no_rebuild(lib_path: &Path, llvm_file: &Path, output_dir: &Path) {
