...
use pecos_engines::{sparse_stab, state_vector, DepolarizingNoise, sim as sim_from};
...
fn main() -> Result<(), PecosError> {
    ...
    let qasm_prog = QasmProgram::from_string("OPENQASM 2.0; qreg q[1]; h q[0]; measure q[0] -> c[0];");
    ...
    let qasm_prog = QasmProgram::from_string("OPENQASM 2.0; qreg q[3]; h q[0]; cx q[0],q[1]; cx q[1],q[2];");
    let results = sim(qasm_prog)
    ...
    let results = sim_from(
        qasm_engine().qasm("OPENQASM 2.0; qreg q[2]; h q[0]; cx q[0],q[1];")
    )
    .quantum(sparse_stab())
    .seed(42)
    .run(100)?;
