...
fn test_hugr_to_llvm_to_execution() -> Result<(), PecosError> {
    ...
    for shot in results.shots.iter() {
        ...
        let c = shot.data.get("c")
            .and_then(|v| match v {
                pecos_engines::shot_results::Data::I64(n) => Some(*n),
                pecos_engines::shot_results::Data::U32(n) => Some(*n as i64),
                _ => None,
            })
            .expect("Expected 'c' register");
            
        let c1 = shot.data.get("c1")
            .and_then(|v| match v {
                pecos_engines::shot_results::Data::I64(n) => Some(*n),
                pecos_engines::shot_results::Data::U32(n) => Some(*n as i64),
                _ => None,
    ...
    assert_eq!(other_outcomes, 0, "Bell state should only produce correlated outcomes (00 or 11)");
    assert!(outcome_00 > 0 || outcome_11 > 0, "Bell state should produce at least one outcome");
    ...
    assert!((ratio_00 - 0.5).abs() < 0.1, "Bell state outcomes should be roughly 50/50, got {:.2}% 00", ratio_00 * 100.0);
...
fn test_single_hadamard_execution() -> Result<(), PecosError> {
    ...
    for shot in results.shots.iter() {
        ...
        let value = shot.data.values().next()
    ...
    assert!((ratio_0 - 0.5).abs() < 0.1, "Hadamard should produce roughly 50/50, got {:.2}% 0", ratio_0 * 100.0);
...
fn test_ghz_state_execution() -> Result<(), PecosError> {
    ...
    for shot in results.shots.iter() {
        ...
        let values: Vec<u32> = shot.data.values()
    ...
    assert_eq!(other_outcomes, 0, "GHZ state should only produce 000 or 111");
    assert!(outcome_000 > 0 || outcome_111 > 0, "GHZ state should produce at least one outcome");
    ...
    assert!((ratio_000 - 0.5).abs() < 0.1, "GHZ state outcomes should be roughly 50/50, got {:.2}% 000", ratio_000 * 100.0);
