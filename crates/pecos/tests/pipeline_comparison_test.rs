...
impl CircuitValidator for BellStateValidator {
    fn validate_quantum_behavior(&self, results: &ShotVec) -> Result<(), String> {
        ...
        for outcome in &outcomes {
            if outcome.len() != 2 {
                return Err(format!("Bell state should have 2 measurements, got {}", outcome.len()));
            }
        ...
        if invalid_outcomes > 0 {
            return Err(format!("Bell state produced {} invalid outcomes (not 00 or 11)", invalid_outcomes));
        }
        
        let total = outcome_00 + outcome_11;
        let ratio_00 = outcome_00 as f64 / total as f64;
        
        // Allow 10% deviation from 50/50 for statistical fluctuations
        if (ratio_00 - 0.5).abs() > 0.1 {
            return Err(format!("Bell state distribution too skewed: {:.1}% 00, {:.1}% 11", 
                              ratio_00 * 100.0, (1.0 - ratio_00) * 100.0));
        }
    ...
    fn extract_outcomes(&self, results: &ShotVec) -> Vec<Vec<u32>> {
        results.shots.iter().map(|shot| {
            // First check if there's a tuple return as Data::Vec
            if let Some(pecos_engines::shot_results::Data::Vec(vec_data)) = shot.data.get("result") {
                // Convert the vector elements to u32 values
                return vec_data.iter().filter_map(|data| {
                    match data {
                        pecos_engines::shot_results::Data::U32(n) => Some(*n),
                        pecos_engines::shot_results::Data::I64(n) => Some(*n as u32),
                        pecos_engines::shot_results::Data::I32(n) => Some(*n as u32),
                        pecos_engines::shot_results::Data::U8(n) => Some(*n as u32),
                        pecos_engines::shot_results::Data::Bool(b) => Some(if *b { 1 } else { 0 }),
                        _ => None,
                    }
                }).collect();
            }
            ...
            for key in &["c", "c1", "result", "m0", "m1"] {
                if let Some(data) = shot.data.get(*key) {
                    let val = match data {
                        ...
                        pecos_engines::shot_results::Data::I64(n) => *n as u32,
                        pecos_engines::shot_results::Data::I32(n) => *n as u32,
                        pecos_engines::shot_results::Data::U8(n) => *n as u32,
                        _ => 0,
                    };
                    values.push(val);
                }
            ...
            if values.is_empty() {
                values = shot.data.values().filter_map(|data| {
                    match data {
                        pecos_engines::shot_results::Data::U32(n) => Some(*n),
                        pecos_engines::shot_results::Data::I64(n) => Some(*n as u32),
                        pecos_engines::shot_results::Data::I32(n) => Some(*n as u32),
...
impl CircuitValidator for HadamardValidator {
    fn validate_quantum_behavior(&self, results: &ShotVec) -> Result<(), String> {
        ...
        for outcome in &outcomes {
            if outcome.len() != 1 {
                return Err(format!("Hadamard should have 1 measurement, got {}", outcome.len()));
            }
        ...
        let ratio_0 = outcome_0 as f64 / total as f64;
        
        // Allow 10% deviation from 50/50
        if (ratio_0 - 0.5).abs() > 0.1 {
            return Err(format!("Hadamard distribution too skewed: {:.1}% 0, {:.1}% 1",
                              ratio_0 * 100.0, (1.0 - ratio_0) * 100.0));
        }
    ...
    fn extract_outcomes(&self, results: &ShotVec) -> Vec<Vec<u32>> {
        results.shots.iter().map(|shot| {
            ...
            let val = shot.data.values().next().map(|data| {
                match data {
                    pecos_engines::shot_results::Data::U32(n) => *n,
                    pecos_engines::shot_results::Data::I64(n) => *n as u32,
                    pecos_engines::shot_results::Data::I32(n) => *n as u32,
    ...
    fn name(&self) -> &str { "Single Hadamard" }
}
...
impl CircuitValidator for GhzStateValidator {
    fn validate_quantum_behavior(&self, results: &ShotVec) -> Result<(), String> {
        ...
        for outcome in &outcomes {
            if outcome.len() != 3 {
                return Err(format!("GHZ state should have 3 measurements, got {}", outcome.len()));
            }
        ...
        if invalid_outcomes > 0 {
            return Err(format!("GHZ state produced {} invalid outcomes (not 000 or 111)", invalid_outcomes));
        }
        
        let total = outcome_000 + outcome_111;
        let ratio_000 = outcome_000 as f64 / total as f64;
        
        // Allow 10% deviation from 50/50
        if (ratio_000 - 0.5).abs() > 0.1 {
            return Err(format!("GHZ state distribution too skewed: {:.1}% 000, {:.1}% 111",
                              ratio_000 * 100.0, (1.0 - ratio_000) * 100.0));
        }
    ...
    fn extract_outcomes(&self, results: &ShotVec) -> Vec<Vec<u32>> {
        results.shots.iter().map(|shot| {
            ...
            let mut values: Vec<u32> = shot.data.values().filter_map(|data| {
                match data {
                    pecos_engines::shot_results::Data::U32(n) => Some(*n),
                    pecos_engines::shot_results::Data::I64(n) => Some(*n as u32),
                    pecos_engines::shot_results::Data::I32(n) => Some(*n as u32),
...
fn run_hugr_llvm_pipeline(hugr_data: &[u8], shots: usize) -> PipelineResult {
    ...
    // Compile and execute
    let execution_result = pecos::hugr::run_hugr_llvm(&hugr_path, Some(shots))
        .and_then(|engine| {
...
fn run_pmir_pipeline(hugr_data: &[u8], shots: usize) -> PipelineResult {
    ...
    // Compile and execute
    let execution_result = pecos::pmir::run_pmir_llvm(&hugr_path, Some(shots), None)
        .and_then(|engine| {
            let compile_time = start_time.elapsed();
            let exec_start = std::time::Instant::now();
            let result = run_sim(engine, shots, Some(42), None, None, None);
...
fn compare_pipelines<V: CircuitValidator>(
    ...
    if hugr_llvm_result.compilation_result.is_err() && pmir_result.compilation_result.is_err() {
        return Err(PecosError::Processing("Both pipelines failed to compile".to_string()));
    }
    ...
    match (&hugr_llvm_result.execution_result, &pmir_result.execution_result) {
        (Some(Ok(hugr_results)), Some(Ok(pmir_results))) => {
            println!("Execution Results:");
            println!("  HUGR-LLVM: {} shots in {:?}ms", hugr_results.len(), hugr_llvm_result.execution_time_ms);
            println!("  PMIR:      {} shots in {:?}ms", pmir_results.len(), pmir_result.execution_time_ms);
    ...
    if comparison_failed {
        Err(PecosError::Processing(format!("Pipeline comparison failed for {}", validator.name())))
    } else {
...
fn compare_outcome_distributions(outcomes1: &[Vec<u32>], outcomes2: &[Vec<u32>]) -> bool {
    ...
    for outcome in counts1.keys() {
        let observed1 = *counts1.get(outcome).unwrap() as f64;
        let observed2 = *counts2.get(outcome).unwrap() as f64;
...
fn analyze_distribution_differences(outcomes1: &[Vec<u32>], outcomes2: &[Vec<u32>]) {
    ...
    for (outcome, count) in &counts1 {
        let percentage = (*count as f64 / total1) * 100.0;
        println!("    {:?}: {} ({:.1}%)", outcome, count, percentage);
    }
    
    println!("  PMIR outcomes:");
    for (outcome, count) in &counts2 {
        let percentage = (*count as f64 / total2) * 100.0;
        println!("    {:?}: {} ({:.1}%)", outcome, count, percentage);
    }
}
...
#[test]
fn test_debug_llvm_ir_comparison() -> Result<(), PecosError> {
    ...
    match pecos::hugr::run_hugr_llvm(&hugr_path, Some(10)) {
        Ok(engine) => {
            ...
            match run_sim(engine, 10, Some(42), None, None, None) {
                Ok(results) => println!("   ✓ HUGR-LLVM execution successful: {} shots", results.len()),
...
fn generate_hugr_llvm_ir(hugr_path: &std::path::Path) -> Result<String, String> {
    // Read the HUGR file
    let hugr_data = std::fs::read(hugr_path)
        .map_err(|e| format!("Failed to read HUGR file: {}", e))?;
...
fn compare_qubit_operations(hugr_ir: &str, pmir_ir: &str) {
    ...
    let hugr_allocations = hugr_ops.iter().filter(|op| op.contains("__quantum__rt__qubit_allocate")).count();
    let pmir_allocations = pmir_ops.iter().filter(|op| op.contains("__quantum__rt__qubit_allocate")).count();
    
    println!("Qubit allocations - HUGR: {}, PMIR: {}", hugr_allocations, pmir_allocations);
    
    let hugr_gates = hugr_ops.iter().filter(|op| op.contains("__quantum__qis__")).count();
    let pmir_gates = pmir_ops.iter().filter(|op| op.contains("__quantum__qis__")).count();
    
    println!("Quantum gates - HUGR: {}, PMIR: {}", hugr_gates, pmir_gates);
}
...
fn test_all_pipeline_comparisons() -> Result<(), PecosError> {
    ...
    if failed_tests.is_empty() {
        ...
        Err(PecosError::Processing(format!("Pipeline comparison failures: {}", failed_tests.join(", "))))
    }
}