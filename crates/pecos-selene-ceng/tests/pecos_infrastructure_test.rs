...
#[test]
fn test_selene_engine_compilation() -> Result<(), PecosError> {
...
"#;
    ...
    for op in &ops {
        *gate_counts.entry(format!("{:?}", op.gate_type)).or_insert(0) += 1;
    }
...
"#;
    ...
    assert!(!outcome_counts.is_empty(), "Should have measurement outcomes");
...
#[test]
fn test_selene_engine_cloning() -> Result<(), PecosError> {
