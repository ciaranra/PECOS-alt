...
PHIR serves as the core IR for PECOS, enabling optimization, analysis, and execution of quantum programs across different backends while maintaining a clean separation between high-level quantum algorithms and low-level implementation details.
...
PHIR aims to be a complete, self-contained IR that can:
...
- **PHIR Core**: Hierarchical SSA IR with Operation → Region → Block structure
...
PHIR's design is heavily influenced by PECOS's SLR (Simple Logical Representation), which demonstrated the power of building complex quantum protocols from simple, composable primitives. This philosophy permeates every level of PHIR:
...
PHIR follows MLIR's philosophy of providing mechanisms, not policies:
...
While adopting MLIR's architecture, PHIR makes quantum programming feel natural:
...
PHIR follows MLIR's hierarchical organization:
...
### 4.2 Parsing with PHIR

PHIR leverages MLIR's hierarchical structure to serve as both AST and IR. During parsing, PHIR uses special "parsing operations" that get progressively lowered:

```rust
// Parsing operations for direct parsing to PHIR
pub enum ParsingOp {
    UnresolvedCall { name: String },    // Forward references
    UnresolvedRef { name: String },      // Variable lookups
    InferType { constraints: Vec<TypeConstraint> }, // Type inference
    ForLoop { init, cond, update, body }, // High-level control flow
}

// These parsing ops exist alongside regular ops in PHIR:
pub enum Operation {
    Parsing(ParsingOp),          // Used during parsing
...
The key insight: **PHIR's hierarchical regions naturally represent both AST structure and SSA form** - no separate representations needed.
...
PHIR automatically selects the best execution strategy:
...
pub trait QuantumParser {
    fn parse(&self, input: &str) -> Result<Module, ParseError>;
}

pub struct ParserRegistry {
    parsers: HashMap<Format, Box<dyn QuantumParser>>,
    
    pub fn parse(&self, format: Format, input: &str) -> Result<Module, Error> {
        let parser = self.parsers.get(&format)?;
        let module = parser.parse(input)?;  // Parse directly to PHIR
        Ok(module)
    }
}
```
...
- Progressive lowering of parsing operations
- Basic optimizations
- MLIR text output
...
|--------|-------------|---------------|
| **MLIR** | Hierarchical regions, SSA form, dialects, passes | **PHIR structure** - Linear SSA, regions, dialect system |
| **PHIR** | Machine operations, explicit data flow, block structures | **Both** - Machine dialect, Result operations, block patterns |
| **HUGR** | Port-based connections, linear types, hierarchical AST | **Direct to PHIR** - Leverages PHIR's hierarchical regions |

### The Combination

- **PHIR** = MLIR's approach applied to quantum computing
- **Unified PHIR** = MLIR's hierarchical structure serves both parsing and transformation needs
...
5. **Unified representation** - PHIR's hierarchical structure handles both parsing and transformations

### Unified PHIR Architecture

The design leverages MLIR's flexibility to use a single representation throughout:

**PHIR** (MLIR-inspired unified IR):
- Hierarchical structure with regions, blocks, and operations
- SSA form with explicit use-def chains
- Parsing operations for handling unresolved references during parsing
- Progressive lowering from parsing ops to machine ops
- Dialect system for extensibility

**Key insight**: MLIR's hierarchical structure is powerful enough to serve as both AST and IR:

- **During Parsing**: Use parsing operations (UnresolvedCall, InferType) within PHIR structure
- **After Lowering**: Only core operations remain (quantum.h, arith.add, etc.)
- **Same Structure**: Regions and blocks naturally represent both AST nesting and control flow
- **Progressive Transformation**: Multi-pass lowering within the same IR

This unified approach eliminates the complexity of maintaining separate representations while providing all the benefits of both AST and SSA form.
...
| Unified PHIR approach | Single representation throughout | Requires careful design of parsing operations |
