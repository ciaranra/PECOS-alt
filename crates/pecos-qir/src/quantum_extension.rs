...
impl QuantumNamingConfig {
    ...
    pub fn with_prefix(prefix: &str) -> Self {
        Self {
            qalloc: format!("{}_qalloc", prefix),
            h: format!("{}_h", prefix),
            cx: format!("{}_cx", prefix),
            measure: format!("{}_measure", prefix),
            x: format!("{}_x", prefix),
            y: format!("{}_y", prefix),
            z: format!("{}_z", prefix),
            rx: format!("{}_rx", prefix),
            ry: format!("{}_ry", prefix),
            rz: format!("{}_rz", prefix),
        ...
        Self: 'a,
    {
        ...
        builder
            .extension_op(ext_id.clone(), "QAlloc".into(), {
                ...
                move |ctx, args| emit_qalloc(ctx, args, &cfg.qalloc).map_err(|e| anyhow::Error::new(e))
            })
            .extension_op(ext_id.clone(), "H".into(), {
                let cfg = config.clone();
                move |ctx, args| emit_single_qubit_gate(ctx, args, &cfg.h).map_err(|e| anyhow::Error::new(e))
            })
            .extension_op(ext_id.clone(), "CX".into(), {
                let cfg = config.clone();
                move |ctx, args| emit_two_qubit_gate(ctx, args, &cfg.cx).map_err(|e| anyhow::Error::new(e))
            })
            .extension_op(ext_id.clone(), "MeasureFree".into(), {
                let cfg = config.clone();
                let names = result_names.clone();
                move |ctx, args| emit_measure(ctx, args, &cfg.measure, &names).map_err(|e| anyhow::Error::new(e))
            })
            .extension_op(ext_id.clone(), "X".into(), {
                let cfg = config.clone();
                move |ctx, args| emit_single_qubit_gate(ctx, args, &cfg.x).map_err(|e| anyhow::Error::new(e))
            })
            .extension_op(ext_id.clone(), "Y".into(), {
                let cfg = config.clone();
                move |ctx, args| emit_single_qubit_gate(ctx, args, &cfg.y).map_err(|e| anyhow::Error::new(e))
            })
            .extension_op(ext_id.clone(), "Z".into(), {
                let cfg = config.clone();
                move |ctx, args| emit_single_qubit_gate(ctx, args, &cfg.z).map_err(|e| anyhow::Error::new(e))
            })
    }
}
...
fn emit_qalloc<'c, H: HugrView<Node = Node>>(
    ...
    let qubit_usize = builder.build_call(func, &[], "qubit_usize")?
        .try_as_basic_value()
        .unwrap_left();
    
    // Convert from usize to i16 for HUGR
    let qubit_i16 = builder.build_int_truncate(
        qubit_usize.into_int_value(),
        i16_type,
        "qubit"
    )?;
...
fn emit_single_qubit_gate<'c, H: HugrView<Node = Node>>(
    ...
    let qubit_usize = builder.build_int_z_extend(
        args.inputs[0].into_int_value(),
        usize_type,
        "qubit_usize"
    )?;
    
    let func_type = llvm_context.void_type().fn_type(&[usize_type.into()], false);
    let func = context.get_extern_func(func_name, func_type)?;
...
fn emit_two_qubit_gate<'c, H: HugrView<Node = Node>>(
    ...
    let control_usize = builder.build_int_z_extend(
        args.inputs[0].into_int_value(),
        usize_type,
        "control_usize"
    )?;
    
    let target_usize = builder.build_int_z_extend(
        args.inputs[1].into_int_value(),
        usize_type,
        "target_usize"
    )?;
    
    let func_type = llvm_context.void_type().fn_type(&[usize_type.into(), usize_type.into()], false);
    let func = context.get_extern_func(func_name, func_type)?;
...
fn emit_measure<'c, H: HugrView<Node = Node>>(
    ...
    let result_id = builder.build_call(alloc_func, &[], "result_id")?
        .try_as_basic_value()
        .unwrap_left();
    ...
    let measurement = builder.build_call(
        measure_func, 
    ...
    builder.build_call(record_func, &[result_id.into(), name_ptr.into()], "")?;
