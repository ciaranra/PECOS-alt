...
impl ResultNameExtractor {
    ...
    pub fn extract_result_names<H: HugrView<Node = Node>>(hugr: &H) -> Result<ResultNameMapping, PecosError> {
        ...
        for node in hugr.nodes() {
            if let Some(op) = hugr.get_optype(node).as_extension_op() {
                if op.def().name() == "result_bool" || op.def().name() == "result_int" || op.def().name() == "result_f64" {
        ...
        for (result_node, result_name) in result_nodes_with_names {
            match Self::find_connected_measurement(hugr, result_node) {
                Ok(Some(measurement_node)) => {
                    result_mapping.insert(measurement_node, result_name);
                }
                Ok(None) => {
                    // No measurement found, skip
                }
                Err(e) => {
                    return Err(PecosError::with_context(e, "Failed to find connected measurement"));
                }
            }
        }
    ...
    fn extract_string_arg_from_result_op(op: &ExtensionOp) -> Option<String> {
        ...
        if let Some(first_arg) = args.first() {
            // TypeArg should be a String variant, extract it
            if let hugr_core::types::TypeArg::String { arg } = first_arg {
    ...
    fn find_connected_measurement<H: HugrView<Node = Node>>(
        ...
        while let Some(current_node) = queue.pop_front() {
            ...
            for input_port in hugr.node_inputs(current_node) {
                if let Some((source_node, _output_port)) = hugr.linked_outputs(current_node, input_port).next() {
