    ...
    match result {
        Ok(output_file) => {
            ...
            if content.contains("__quantum__qis__h__body__hugr") {
                ...
                println!("⚠ HUGR convention: No __hugr suffixed functions found, checking for integer-based calls");
                // The functions might be called directly with integer parameters
                assert!(content.contains("__quantum__qis__"), "Should contain quantum function calls");
            }
...
fn test_qir_convention_compilation() {
    ...
    let compiler = Compiler::new()
        .with_quantum_naming(QuantumLlvmConvention::Qir);
    ...
    match result {
        Ok(output_file) => {
            ...
            if content.contains("__quantum__qis__h__body") && !content.contains("__quantum__qis__h__body__hugr") {
...
fn test_both_conventions_produce_different_output() {
    ...
    let hugr_compiler = Compiler::new()
        .with_quantum_naming(QuantumLlvmConvention::Hugr);
    
    // Compile with QIR convention  
    let qir_compiler = Compiler::new()
        .with_quantum_naming(QuantumLlvmConvention::Qir);
    ...
    match (hugr_result, qir_result) {
        (Ok(hugr_output), Ok(qir_output)) => {
            let hugr_content = fs::read_to_string(&hugr_output).expect("Failed to read HUGR output");
