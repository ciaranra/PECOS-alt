...
fn test_hugr_rotation_gate_structure() {
    ...
    match hugr_to_past_ron(hugr_json) {
        Ok(ron) => {
            ...
            assert!(ron.contains("RX(1.5708)"), "Expected RX(1.5708) in RON output");
        },
...
fn test_edge_based_angle_passing() {
    ...
    match hugr_to_past_ron(hugr_json) {
        Ok(ron) => {
            ...
            assert!(ron.contains("RZ(3.14159)"), "Expected RZ(3.14159) in RON output");
            
            // Also check that we have a Const node with the pi value
            assert!(ron.contains("Const(Float(3.14159))"), "Expected Const(Float(3.14159)) in RON output");
        },
        Err(e) => panic!("Failed to convert to RON: {:?}", e),
    }
}