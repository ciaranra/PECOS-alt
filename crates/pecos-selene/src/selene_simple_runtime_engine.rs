...
use std::{any::Any, collections::BTreeMap, path::PathBuf, io::Write};
...
impl SeleneSimpleRuntimeEngine {
    ...
    fn load_plugin_interface(&mut self) -> Result<(), PecosError> {
        log::info!("Loading Selene Simple Runtime plugin from {:?}", self.runtime_library_path);
        
        let plugin_interface = RuntimePluginInterface::new_from_file(&self.runtime_library_path)
            .map_err(|e| SeleneError::RuntimeError(format!("Failed to load runtime plugin: {}", e)))?;
    ...
    fn load_interface_plugin(&mut self) -> Result<(), PecosError> {
        let program = self.program.as_ref()
            .ok_or_else(|| SeleneError::CompilationError("No Interface Plugin program available".to_string()))?;
        
        println!("[SeleneSimpleRuntimeEngine] Loading Interface Plugin ({} bytes)", program.plugin.len());
        ...
        temp_file.write_all(&program.plugin)
            .map_err(|e| SeleneError::RuntimeError(format!("Failed to write plugin bytes: {}", e)))?;
        ...
        let library = unsafe { 
            Library::new(temp_path)
                .map_err(|e| SeleneError::RuntimeError(format!("Failed to load Interface Plugin library: {}", e)))?
        };
    ...
    fn load_ffi_stubs() -> Result<(), PecosError> {
        ...
        let stub_lib_paths = vec![
            ...
            std::env::current_dir().unwrap_or_default().join("target").join("debug").join("libselene_correct_stubs.so"),
            // Current working directory
            std::env::current_dir().unwrap_or_default().join("libselene_correct_stubs.so"),
            // Next to this crate
            std::env::current_dir().unwrap_or_default().join("crates").join("pecos-selene").join("libselene_correct_stubs.so"),
        ];
    ...
    fn get_operations_from_selene_runtime(&mut self) -> Result<ByteMessage, PecosError> {
        ...
        println!("[SeleneSimpleRuntimeEngine] program.is_some()={}, shot_count={}", self.program.is_some(), self.shot_count);
        ...
        if self.program.is_some() {
            ...
            println!("[SeleneSimpleRuntimeEngine] Executing Interface Plugin for shot {}", self.shot_count);
        ...
        let runtime = self.runtime.as_mut()
            .ok_or_else(|| SeleneError::RuntimeError("No runtime instance available".to_string()))?;
    ...
    fn convert_selene_operations_to_pecos(&mut self, operations: &[Operation]) -> Result<ByteMessage, PecosError> {
        ...
        for operation in operations {
            match operation {
                Operation::RXYGate { qubit_id, theta, phi } => {
                    log::debug!("Converting RXY gate: qubit={}, theta={}, phi={}", qubit_id, theta, phi);
                    // Use R1XY for RXY gates (closest PECOS equivalent)
                    self.message_builder.add_r1xy(*theta, *phi, &[*qubit_id as usize]);
                }
                ...
                Operation::RZZGate { qubit_id_1, qubit_id_2, theta } => {
                    log::debug!("Converting RZZ gate: qubit1={}, qubit2={}, theta={}", qubit_id_1, qubit_id_2, theta);
                    self.message_builder.add_rzz(*theta, &[*qubit_id_1 as usize], &[*qubit_id_2 as usize]);
                }
                Operation::Measure { qubit_id, result_id } => {
                    log::debug!("Converting Measure: qubit={}, result_id={}", qubit_id, result_id);
                ...
                Operation::MeasureLeaked { qubit_id, result_id } => {
                    log::debug!("Converting MeasureLeaked: qubit={}, result_id={}", qubit_id, result_id);
                    // MeasureLeaked in PECOS - measures if qubit is in leaked state
                    self.message_builder.add_measure_leakages(&[*qubit_id as usize]);
                }
            }
        }
        ...
        log::debug!("Converted {} Selene operations to PECOS ByteMessage", operations.len());
        Ok(message)
    }
}
...
impl ClassicalEngine for SeleneSimpleRuntimeEngine {
    ...
    fn generate_commands(&mut self) -> Result<ByteMessage, PecosError> {
        ...
        eprintln!("[GENERATE_COMMANDS] get_operations_from_selene_runtime returned: {:?}", result.as_ref().map(|_| "Ok").unwrap_or("Err"));
        result
    }
    ...
    fn reset(&mut self) -> Result<(), PecosError> {
        ...
        if let Some(runtime) = self.runtime.as_mut() {
            ...
            runtime.shot_start(self.shot_count, 42 + self.shot_count)
