...
impl SeleneInProcessEngine {
    ...
    fn load_interface_plugin(&mut self) -> Result<(), PecosError> {
        ...
        let selene_lib_path = std::env::var("SELENE_LIB_PATH")
            .unwrap_or_else(|_| {
                // Try to find it in the Python package
                let python_site = std::process::Command::new("python")
                    .args(&["-c", "import selene_sim; import os; print(os.path.dirname(selene_sim.__file__))"])
                    .output()
                    .ok()
        ...
        unsafe {
            ...
            let entry: libloading::Symbol<unsafe extern "C" fn()> = lib_static.get(b"qmain")
