...
pub fn load_hugr_013_package(hugr_bytes: &[u8]) -> Result<Package, crate::SeleneError> {
    ...
    if hugr_bytes.len() >= 10 && &hugr_bytes[0..8] == b"HUGRiHJv" {
        ...
        log::debug!("HUGR envelope: format_byte={}, flags={}, compressed={}", format_byte, flags, compressed);
        ...
        if format_byte == 2 {
            ...
            return Err(crate::SeleneError::HugrError(
                format!("Unknown HUGR envelope format: {}", format_byte)
            ));
        }
        ...
        let json_bytes = if compressed {
            ...
            match zstd::decode_all(payload) {
                Ok(decompressed) => {
                    log::debug!("Decompressed {} bytes to {} bytes", payload.len(), decompressed.len());
                    decompressed
                },
                Err(e) => return Err(crate::SeleneError::HugrError(
                    format!("Failed to decompress HUGR envelope: {}", e)
                )),
            }
        } else {
            payload.to_vec()
        ...
        if json_value.is_object() && json_value.get("modules").is_some() && json_value.get("extension_reqs").is_none() {
            ...
            if has_extensions {
                eprintln!("DEBUG: Found 'extensions' field in envelope, renaming to 'extension_reqs'");
                if let Some(obj) = json_value.as_object_mut() {
                    if let Some(ext_value) = obj.remove("extensions") {
                        obj.insert("extension_reqs".to_string(), ext_value);
                        eprintln!("DEBUG: Successfully renamed 'extensions' to 'extension_reqs' in envelope");
                    }
                }
            } else {
            ...
            Err(e) => {
                ...
                if let Some(modules) = json_value.get("modules").and_then(|v| v.as_array()) {
                    if let Some(first_module) = modules.first() {
                        ...
                        let hugr: Hugr = serde_json::from_value(first_module.clone())
                            .map_err(|e| crate::SeleneError::HugrError(format!("Failed to deserialize module as HUGR: {}", e)))?;
                        
                        // Create a package with single module
                        Package::from_hugr(hugr)
                            .map_err(|e| crate::SeleneError::HugrError(format!("Failed to create package from HUGR: {}", e)))
                    } else {
                        Err(crate::SeleneError::HugrError("No modules found in JSON".to_string()))
                    }
                } else {
                    // Try loading as single HUGR
                    let hugr: Hugr = serde_json::from_value(json_value)
                        .map_err(|e| crate::SeleneError::HugrError(format!("Failed to deserialize as HUGR: {}", e)))?;
        ...
        eprintln!("DEBUG: Raw JSON has modules: {}", json_value.get("modules").is_some());
        eprintln!("DEBUG: Raw JSON has extension_reqs: {}", json_value.get("extension_reqs").is_some());
        eprintln!("DEBUG: Raw JSON has extensions: {}", json_value.get("extensions").is_some());
        ...
        if json_value.is_object() 
            ...
            if let Some(modules) = json_value.get("modules").and_then(|v| v.as_array()) {
                ...
                if let Some(first_module) = modules.first() {
                            ...
                            if let Some(op) = node.get("op") {
                                ...
                                if op == "Extension" {
                                    ...
                                    if let Some(extension_name) = node.get("extension_name") {
                                        eprintln!("DEBUG: Node {} is Extension with name: {:?}", i, extension_name);
                                    }
                                    ...
                                    if i == 9 || i == 10 || i == 11 || i == 12 {
                                        eprintln!("DEBUG: Full node {} structure: {}", i, serde_json::to_string_pretty(node).unwrap_or_default());
                                    }
                                }
                            }
                        }
                    ...
                    Package::from_hugr(empty_hugr)
                        .map_err(|e| crate::SeleneError::HugrError(
                            format!("Failed to create package from HUGR: {}", e)
                        ))
                } else {
                    Err(crate::SeleneError::HugrError("No modules found in guppylang format".to_string()))
                }
            } else {
                Err(crate::SeleneError::HugrError("Invalid guppylang format - modules is not an array".to_string()))
            }
        } else {
                    ...
                    match serde_json::from_value::<Hugr>(json_value) {
                        ...
                        Err(hugr_err) => {
                            Err(crate::SeleneError::HugrError(
                                format!("Failed to load as Package: {}, or as HUGR: {}", e, hugr_err)
                            ))
                        }
                    }
                }
