...
use pecos_engines::{
    ByteMessage, ByteMessageBuilder, ClassicalEngine, ControlEngine, 
    Engine, EngineStage, Shot, Data,
};
            ...
            if let Ok((mut stream, _)) = listener.accept() {
                ...
                for line in data.lines() {
                    if let Some((_tag, rest)) = line.split_once(':') {
                        if let Some((typ, value_str)) = rest.split_once(':') {
                            if let Some((name, value)) = value_str.split_once('=') {
                                // Parse based on type
                                let parsed_value = match typ {
                                    "BOOL" => {
                                        value.parse::<bool>().ok().map(Data::Bool)
                                    },
                                    "INT" => {
                                        value.parse::<i64>().ok().map(Data::I64)
                                    },
                                    _ => None
                                };
...
impl SeleneLibraryEngine {
    ...
    fn start_execution(&mut self) -> Result<(), PecosError> {
        ...
        self.execution_thread = Some(thread::spawn(move || {
            ...
            if result == 0 {
                ...
                Err(PecosError::Processing(format!("Program exited with code {}", result)))
            }
        }));
    ...
    fn get_pending_operations(&mut self) -> Option<ByteMessage> {
        self.callback_state.lock().unwrap().operation_queue.pop_front()
    }
    
    /// Provide measurements to the Bridge
    fn provide_measurements(&mut self, measurements: ByteMessage) {
        self.callback_state.lock().unwrap().measurement_queue.push_back(measurements);
    }
...
impl ControlEngine for SeleneLibraryEngine {
    ...
    fn start(&mut self, _input: ()) -> Result<EngineStage<ByteMessage, Shot>, PecosError> {
        ...
        if let Some(ops) = self.wait_for_operations(5000) {
            ...
            Err(PecosError::Processing("Timeout waiting for initial operations".to_string()))
        }
    }
    ...
    fn continue_processing(
        ...
        if let Some(ops) = self.wait_for_operations(1000) {
            ...
            log::info!("Execution complete with {} results", self.final_results.len());
            Ok(EngineStage::Complete(self.get_results()?))
        } else {
    ...
    fn process(&mut self, _input: Self::Input) -> Result<Self::Output, PecosError> {
        ...
        match self.start(())? {
            ...
            EngineStage::NeedsProcessing(ops) => {
                ...
                Err(PecosError::Processing("Unexpected NeedsProcessing state".to_string()))
            }
        }
    }
