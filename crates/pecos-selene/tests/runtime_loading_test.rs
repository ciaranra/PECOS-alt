    ...
    match builder.build() {
        ...
        Err(e) => {
            ...
            if err_str.contains("not found") || err_str.contains("No such file") {
                println!("Selene runtime library not found (expected if not installed): {}", e);
            } else {
                // Unexpected error
...
fn test_runtime_with_explicit_path() {
    ...
    println!("No Selene runtime libraries found in test paths (this is OK if Selene is not installed)");
}