...
impl JitExecutor {
    ...
    fn transform_opaque_qir_to_integer(&self, llvm_ir: &str) -> String {
        ...
        for line in llvm_ir.lines() {
            ...
            if trimmed.starts_with("declare") {
                ...
                new_line = new_line.replace("(%Qubit*, %Qubit*)", "(i64, i64)");
                new_line = new_line.replace("(%Qubit*, %Result*)", "(i64, i64)");

                // Handle array allocation
                if new_line.contains("@__quantum__rt__qubit_allocate_array") {
                    // Skip the original declaration - we don't need it
                    continue;
                }

                // Handle array release
                if new_line.contains("@__quantum__rt__qubit_release_array") {
                    // Skip - we'll handle individual qubit release
                    continue;
                }

                // Handle single qubit allocation
                if new_line.contains("@__quantum__rt__qubit_allocate()") {
                    // Skip - we'll use simple integer assignment
                    continue;
                }
                ...
                if new_line.contains("@__quantum__qis__mz__body") {
                    ...
                    // Both return-style and two-parameter forms map to the same signature
                    new_line = "declare void @__quantum__qis__m__body(i64, i64)".to_string();
                }

                if new_line.contains("@__quantum__rt__qubit_release") {
                    // Map release to a no-op
                    continue;
                }

                if new_line.contains("@__quantum__rt__result_record_output") {
                    // This records output - skip it
                    continue;
                }
            ...
            // Handle function body
                ...
                if trimmed == "}" {
                    ...
                    // Handle qubit release - skip it (no-op)
                    else if new_line.contains("@__quantum__rt__qubit_release") {
                        // Skip qubit release calls - we don't need them for operation collection
                        continue;
                    }
        ...
        if llvm_ir.contains("ModuleID") &&
           ...
           llvm_ir.contains("define") {
            ...
            for line in processed.lines() {
                // Skip attributes lines - we detect EntryPoint before preprocessing
    ...
    fn find_entry_point_function(&self, llvm_ir: &str) -> Option<String> {
        ...
        let mut function_attrs = std::collections::HashMap::new();

        // First pass: find all attribute definitions
        for line in llvm_ir.lines() {
            let trimmed = line.trim();
    ...
    fn try_execute_main_function(&self, execution_engine: &ExecutionEngine, entry_point: Option<String>) -> Result<(), String> {
        ...
        // Fall back to qmain(i64) signature
...
entry:
  ...
  ret i64 %result
    ...
    #[test]
    fn test_jit_caching_disabled() {
        env_logger::try_init().ok();
        info!("Testing that JIT caching is properly disabled");
...
entry:
  ret i64 42
        ...
        // Execute twice - with caching disabled, both should compile fresh
        ...
        // Verify caching is disabled - should see no cache hits
        let (total_compilations, cache_hits, hit_rate) = executor.get_execution_stats();

        // With caching disabled, we expect:
