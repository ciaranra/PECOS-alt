...
impl QisInterfaceBuilder for JitInterfaceBuilder {
    fn build_from_qis_program(
        ...
        executor.execute_llvm_ir(&llvm_ir).map_err(|e| {
            PecosError::Processing(format!("Failed to execute QIS program with JIT: {e}"))
        })
    }
    ...
    fn build_from_hugr_program(
        ...
        #[cfg(not(feature = "hugr"))]
        {
            ...
            Err(PecosError::Processing(
                ...
                Please enable the 'hugr' feature in pecos-qis-jit to use HUGR compilation."
                    .to_string(),
            ))
        }
    }
