# PECOS PMIR - MLIR-Inspired Quantum Compiler IR

PECOS PMIR (PECOS Middle-level Intermediate Representation) is an MLIR-inspired compiler infrastructure for quantum programs, providing a unified representation from parsing through execution.

## Overview

PMIR follows MLIR's design philosophy where everything is an Operation. This provides a single, hierarchical representation throughout the compilation pipeline:

```
Source → PMIR (parsing ops) → PMIR (high-level) → PMIR (low-level) → Execution
         ↓                     ↓                   ↓
...
Key features:
- **Unified representation**: No separate AST - parse directly to PMIR
...
PMIR follows key design principles inspired by PECOS's SLR (Simple Logical Representation):
...
PMIR provides the mechanisms for representing quantum programs. Users define the policies (specific QEC schemes, optimization strategies) through attributes and passes.

```rust
// PMIR provides the mechanism (operations, regions, attributes)
...
These principles ensure PMIR remains flexible enough for research while providing structure for production use.

## Architecture

### Core Structure

PMIR follows MLIR's hierarchical structure:
