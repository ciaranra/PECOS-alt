...
- **PHIR-RON** = Rust-native serialization for tools
...
```
PHIR (in-memory IR) ←→ PHIR-JSON (human-readable) ←→ PHIR-RON (Rust-native)
         ↑                    ↑                           ↑
    Fast, mutable      Stable, versioned            Compact, typed
```
...
│               │   │               │   │               │
│ Human-first   │   │ Rust-native   │   │ MLIR standard │
│ Stable API    │   │ Compact       │   │ Tool chain    │
...
### PHIR-RON (Tool Interface)

**Purpose**: Rust-native serialization for tools and debugging

**Benefits**:
- Natural enum representation
- Compact syntax
