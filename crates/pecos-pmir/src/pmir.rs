...
MLIR-inspired linear SSA intermediate representation that shares operations with PAST.

Key design principles:
1. MLIR-style hierarchical organization: Module → Function → Region → Block
2. SSA form with explicit use-def chains
3. Shared operation and type system with PAST
...
PMIR is essentially "PAST in linear SSA form" - same operations, different organization.
*/
...
/// PMIR Module - convenience wrapper around `ModuleOp`
///
...
/// PMIR Function - convenience wrapper around `FuncOp`
///
