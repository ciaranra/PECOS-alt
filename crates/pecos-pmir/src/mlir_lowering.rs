/*!
PMIR to MLIR Lowering

This module converts PMIR (PECOS Middle-level IR) to MLIR text format.
...
/// Convert PMIR Module to MLIR text
pub fn lower_pmir_to_mlir(module: &Module, _config: &PMIRConfig) -> Result<MlirModule> {
...
/// Convert a PMIR function to MLIR text
...
/// Convert PMIR type to MLIR type string
...
/// Convert PMIR instruction to MLIR text with SSA value mapping
fn convert_instruction_to_mlir_with_mapping(
    instruction: &crate::pmir::Instruction,
            ...
            match quantum_op {
                ...
                QuantumOp::H => {
                    ...
                    let operand = instruction
                        .operands
                        .first()
                        .ok_or_else(|| PMIRError::internal("H gate missing operand"))?;
                ...
                QuantumOp::CX => {
                    ...
                    if instruction.operands.len() < 2 {
                        return Err(PMIRError::internal("CX gate needs 2 operands"));
                    }
                ...
                QuantumOp::Measure => {
                    ...
                    let operand = instruction
                        .operands
                        .first()
                        .ok_or_else(|| PMIRError::internal("Measure missing operand"))?;
                    let qubit_id = resolve_ssa(operand.id);
...
/// Convert PMIR terminator to MLIR text
fn convert_terminator_to_mlir(terminator: &crate::pmir::Terminator) -> Result<String> {
...
/// Convert PMIR Module to MLIR text string
///
/// This is a convenience wrapper around `lower_pmir_to_mlir` that returns the MLIR text directly
pub fn pmir_to_mlir(module: &Module, config: &PMIRConfig) -> Result<String> {
    let mlir_module = lower_pmir_to_mlir(module, config)?;
