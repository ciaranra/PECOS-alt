/*!
Core operation definitions for PMIR

This module defines the complete operation set for PMIR, including:
...
- Parsing operations (for direct parsing to PMIR)
...
pub struct CustomOp {
    ...
    pub attributes: HashMap<String, crate::pmir::AttributeValue>,
}
...
pub enum ValueRef {
    /// SSA value reference (for PMIR)
    SSA(SSAValue),
    /// Variable name reference (for PAST)
    Variable(String),
    /// Immediate constant
