...
use crate::engine_builders::{
    ...
    PyQasmEngineBuilder, PyQasmProgram, PyQasmSimBuilder, PyQisControlEngineBuilder,
    PyQisControlSimBuilder, PyQisEngineBuilder, PyQisProgram, PyQisSimBuilder,
};
...
fn is_guppy_function(py: Python, obj: &Py<PyAny>) -> PyResult<bool> {
    ...
    let _guppylang = match py.import(pyo3::intern!(py, "guppylang")) {
        Ok(module) => module,
        Err(_) => {
...
pub(crate) enum SimBuilderInner {
    ...
    QisControl(PyQisControlSimBuilder),      // New unified QIS/HUGR control engine
        ...
        Python::attach(|py| {
            match &mut self.inner {
                SimBuilderInner::Qasm(sim_builder) => {
                    if let Ok(mut qasm_engine) = engine_builder.extract::<PyQasmEngineBuilder>(py) {
                        ...
                        if let Some(existing_engine) = existing_engine_lock.as_ref() {
                            if existing_engine.has_source() && !qasm_engine.inner.has_source() {
                                if let Some(program) = existing_engine.get_program() {
                ...
                SimBuilderInner::QisControl(sim_builder) => {
                    if let Ok(qis_control_engine) = engine_builder.extract::<PyQisControlEngineBuilder>(py) {
                        sim_builder.engine_builder = Arc::new(Mutex::new(Some(qis_control_engine.inner)));
                        Ok(PySimBuilder {
                            inner: self.inner.clone(),
                        })
                    } else {
                        Err(PyTypeError::new_err(
                            "For QIS Control Engine programs, classical() requires a QisControlEngineBuilder",
                        ))
                    }
                }
    ...
    fn run(&self, shots: usize) -> PyResult<crate::shot_results_bindings::PyShotVec> {
        ...
        match &self.inner {
            ...
            SimBuilderInner::QisControl(builder) => {
                // Implementation for QIS Control Engine (similar to LLVM but uses QisControlEngineBuilder)
                let mut builder_lock = builder.engine_builder.lock().unwrap();
                let engine_builder = builder_lock
                    .take()
                    .ok_or_else(|| PyRuntimeError::new_err("Builder already consumed"))?;

                // Pass the engine builder directly to SimBuilder
    ...
    fn build(&self) -> PyResult<Py<PyAny>> {
        ...
        Python::attach(|py| {
            match &self.inner {
                ...
                // LLVM, QisControl don't have build() methods in current implementation
                ...
                SimBuilderInner::QisControl(_) => Err(PyRuntimeError::new_err(
                    "QIS Control Engine simulation does not support build() yet - use run() directly",
                )),
                SimBuilderInner::Empty => Err(PyRuntimeError::new_err(
