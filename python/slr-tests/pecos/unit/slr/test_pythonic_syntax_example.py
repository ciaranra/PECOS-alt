...
from pecos.slr import Main, CReg, If, SlrConverter

def test_pythonic_syntax_example():
    ...
    guppy_code = SlrConverter(prog).guppy(module_name="pythonic_example")
    ...
    assert "c[6] = not ((c[4] & c[5] ^ (c[0] | c[3])))" in guppy_code
