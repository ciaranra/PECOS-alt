...
def test_function_returns_unconsumed_qubits():
...
def test_partial_array_return():
    ...
    assert "array(q[0], q[2])" in guppy or "array(_q_0, _q_2)" in guppy


def test_multiple_partial_returns():
...
def test_no_return_when_all_consumed():
...
def test_qec_pattern_with_partial_returns():
