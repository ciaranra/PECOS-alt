...
def test_consecutive_gate_applications():
...
def test_register_wide_generates_loop():
...
def test_mixed_individual_and_register_wide():
...
def test_loop_in_function():
    ...
    assert "def test_loop_generation_apply_hadamards" in guppy_code or "def apply_hadamards" in guppy_code
    assert "for i in range(0, 4):" in guppy_code
    assert "quantum.h(q[i])" in guppy_code


def test_different_gates_separate_loops():
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("✅ All loop generation tests passed!")