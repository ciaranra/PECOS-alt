...
class TestPartialConsumption:
    ...
    def test_measure_ancillas_preserve_data(self):
    ...
    def test_consume_subset_of_qubits(self):
        ...
        assert ("c_first[0] = quantum.measure(" in guppy_code or 
                "c_first_0 = quantum.measure(" in guppy_code)
        assert ("c_first[1] = quantum.measure(" in guppy_code or
                "c_first_1 = quantum.measure(" in guppy_code)
        assert ("c_first[2] = quantum.measure(" in guppy_code or
                "c_first_2 = quantum.measure(" in guppy_code)
        ...
        assert ("c_second[0] = quantum.measure(" in guppy_code or
                "c_second_0 = quantum.measure(" in guppy_code)
        assert ("c_second[1] = quantum.measure(" in guppy_code or
                "c_second_1 = quantum.measure(" in guppy_code)
        assert ("c_second[2] = quantum.measure(" in guppy_code or
                "c_second_2 = quantum.measure(" in guppy_code)
    
    def test_function_returning_quantum_resources(self):
        ...
        assert "return data" in guppy_code
        
        # Check function call captures returned resources
        assert "data = test_partial_consumption_stabilizer_measurement(ancilla, data, syndrome)" in guppy_code
        
        # Should measure ancilla
        assert "syndrome" in guppy_code
        assert "quantum.measure" in guppy_code
    
    def test_consecutive_measurements_optimization(self):
    ...
    def test_mixed_destination_measurements(self):
    ...
    def test_array_unpacking_with_gates(self):
        ...
        # Should unpack before first measurement
        assert "q_0, q_1, q_2 = q" in guppy_code
        
        # Gates should use unpacked names
        assert "quantum.x(q_1)" in guppy_code
        
        # Measurements use unpacked names
        assert "c_0 = quantum.measure(q_0)" in guppy_code
    
    def test_single_element_array_unpacking(self):
        ...
        assert ("single_0, = single" in guppy_code or  # Pre-allocated with unpacking
                "single_0 = quantum.qubit()" in guppy_code)  # Dynamic allocation
        assert ("result_reg[0] = quantum.measure(single_0)" in guppy_code or
                "result[0] = quantum.measure(single_0)" in guppy_code)
    
    @pytest.mark.optional_dependency
    def test_hugr_compilation(self):
