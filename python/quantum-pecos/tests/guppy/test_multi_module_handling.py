...
def count_modules_in_hugr(hugr_str: str) -> tuple[int, list[str]]:
    ...
    try:
        ...
        for module in modules:
            ...
            for node in nodes:
                if (node.get("op") == "FuncDefn" and
                    "name" in node and
                    node["name"] != "__main__"):  # Skip main module
...
def test_single_module_baseline():
...
@pytest.mark.skipif(not GUPPY_AVAILABLE, reason="guppylang not available")
def test_multiple_functions_compilation():
...
def test_compiler_comparison_simple():
...
@pytest.mark.skipif(not GUPPY_AVAILABLE, reason="guppylang not available")
def test_hugr_structure_analysis():
    ...
    print(f"HUGR string format analysis:")
