...
def test_existing_hugr_files_parity():
    ...
    for hugr_file in test_data_dir.glob("*.hugr"):
        ...
        # Assume existing files are in JSON format (generated by our script)
        # We'll need to decode them for comparison
        try:
            hugr_json = hugr_bytes.decode('utf-8')
        except UnicodeDecodeError:
            # If it's binary format, skip this test
            pytest.skip(f"{hugr_file.name} is in binary format, can't compare")

        # For Selene, we need binary format but our files are JSON
        # Skip this test since we can't convert JSON back to binary easily
        pytest.skip(f"Skipping {hugr_file.name} - need guppylang to generate binary format")
