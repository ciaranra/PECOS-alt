    ...
    **kwargs: Any,  # noqa: ANN401
    ...
    try:
        ...
        if "HUGR version incompatibility" in str(e):
            if verbose:
                print(
                    "[WARNING] HUGR version incompatibility detected, switching to Selene backend",
                )
            ...
            try:
                ...
                if verbose:
                    print(
                        f"[PASS] Compilation completed with Selene backend in {compilation_time:.4f}s",
                    )
    ...
    if used_selene:
        ...
        if verbose:
            print(
                "[INFO] Executing through Selene runtime with PECOS simulation infrastructure",
            )
        ...
        def count_qubits_in_llvm_ir(llvm_content):
            ...
            try:
                ...
                alloc_count = llvm_content.count(
        ...
        try:
            ...
            if used_selene:
                ...
                if llvm_file.exists():
                    ...
                    if verbose:
                        print(
                            f"[INFO] Detected {n_qubits} qubits from LLVM IR analysis",
                        )

                    # Use LLVM IR file for direct execution
                    engine_builder = (
                        selene_engine().llvm_file(str(llvm_file)).qubits(n_qubits)
                    )
                    ...
                    engine_builder = (
                        selene_engine().hugr_file(str(hugr_file)).qubits(n_qubits)
                    )
                else:
                    msg = f"No LLVM or HUGR file found in {output_dir}"
                ...
                try:
                    ...
                    if verbose:
                        print(
                            f"[INFO] Could not analyze QIR file, using default {n_qubits} qubits",
                        )

                # For regular path, use the QIR file as LLVM
                engine_builder = (
                    selene_engine().llvm_file(str(qir_file)).qubits(n_qubits)
                )
            ...
            if max_qubits is not None and n_qubits > max_qubits:
                if verbose:
                    print(
                        f"[WARNING] Detected {n_qubits} qubits but max_qubits is {max_qubits}, using {max_qubits}",
                    )
                n_qubits = max_qubits
                ...
                try:
                    ...
                    for gate in non_clifford_gates:
                        if gate in llvm_content:
                            ...
                            if verbose:
                                print(
                                    f"[INFO] Detected non-Clifford gate {gate}, using state vector simulator",
                                )
                            break
                except:
                    pass
            ...
            try:
                ...
                if hasattr(sim_results, "to_dict") and callable(sim_results.to_dict):
                    try:
                        ...
                        if isinstance(dict_results, dict):
                            ...
                            if "result" in dict_results:
                                ...
                                result_keys = [
                                    k for k in dict_results if k.startswith("result_")
                                ]
                                if result_keys:
                                    if len(result_keys) == 1:
                                        ...
                                        if verbose:
                                            print(
                                                f"[DEBUG] Using single result key: {result_keys[0]}",
                                            )
                                    else:
                                        ...
                                        try:
                                            ...
                                            if hasattr(
                                                ...
                                                sig = inspect.signature(
                                                    guppy_function.wrapped.python_func,
                                                )
                                                return_type = sig.return_annotation
                                                function_returns_single_bool = (
                                                    return_type == bool
                                                )
                                                if verbose:
                                                    print(
                                                        f"[DEBUG] Function return type: {return_type}, single bool: {function_returns_single_bool}",
                                                    )
                                        except Exception as e:
                                            if verbose:
                                                print(
                                                    f"[DEBUG] Could not determine return type: {e}",
                                                )
                                        ...
                                        if function_returns_single_bool:
                                            ...
                                            if verbose:
                                                print(
                                                    f"[DEBUG] Using first result key for single bool function: {sorted(result_keys)[0]}",
                                                )
                                        else:
                                            # Combine into tuples for multi-value functions
                                            sorted_keys = sorted(result_keys)
                                            shot_count = len(
                                                dict_results[sorted_keys[0]],
                                            )
                                            results = []
                                            ...
                                            if verbose:
                                                print(
                                                    f"[DEBUG] Combined {len(sorted_keys)} result keys into tuples",
                                                )
                                else:
                                    if verbose:
                                        print(
                                            f"[DEBUG] No result keys found in dict: {list(dict_results.keys())}",
                                        )
                            if verbose and converted:
                                print(
                                    f"[DEBUG] Extracted {len(results)} results from dict",
                                )
                    except Exception as e:
                        if verbose:
                ...
                if not converted:
                    try:
                        ...
                        if verbose:
                            print(
                                f"[DEBUG] Direct list conversion worked, got {len(results)} results",
                            )
                    except Exception as e:
                        if verbose:
                ...
                if not converted and hasattr(sim_results, "__len__"):
                    try:
                        ...
                        if verbose:
                            print(
                                f"[DEBUG] Index-based conversion worked, got {len(results)} results",
                            )
                    except Exception as e:
                        if verbose:
                ...
                if not converted:
                    if verbose:
                        print(
                            "[WARNING] Could not convert ShotVec, returning raw object",
                        )
                    results = sim_results
            ...
            if verbose:
                ...
                print(
                    "[INFO] This may be due to missing plugin support or incomplete implementation",
                )

            # If Selene execution fails due to missing implementation, fallback to regular path
            if "No backend for selene execution" in str(e) or "not implemented" in str(
                e,
            ):
                if verbose:
                    print(
                        "[INFO] Selene execution not yet implemented, falling back to regular QIR path",
                    )
        ...
        try:
            ...
            if hasattr(guppy_function, "wrapped") and hasattr(
                ...
                if return_type == bool and results and isinstance(results[0], tuple):
                    if verbose:
                        print(
                            f"Post-processing: Extracting bool from {len(results[0])}-tuples",
                        )
                ...
                # If function returns a specific tuple size, ensure we match it
                    ...
                    if (
                        ...
                        and len(results[0]) != expected_size
                    ):
                        if verbose:
                            print(
                                f"Post-processing: Adjusting tuple size from {len(results[0])} to {expected_size}",
                            )
                        # Take the last N elements to match expected size
                        results = [
                            r[-expected_size:] if len(r) >= expected_size else r
                            for r in results
                        ]
        except Exception as e:
