...
class QirEngineWrapper:
    ...
    def __init__(self):
        ...
        if not QIR_ENGINE_AVAILABLE:
            raise ImportError("PECOS QIR engine not available")
    ...
    def load_qir_file(self, qir_file_path: Path, shots: int = 1000) -> None:
        ...
        if not qir_file_path.exists():
            raise FileNotFoundError(f"QIR file not found: {qir_file_path}")
    ...
    def execute(self) -> Dict[str, Any]:
        ...
        if self.engine is None:
            raise RuntimeError("No QIR file loaded. Call load_qir_file() first.")
    ...
    def execute_qir_file(self, qir_file_path: Path, shots: int = 1000) -> Dict[str, Any]:
        """
...
def get_qir_engine_info() -> Dict[str, Any]:
    ...
    return {
        ...
        'description': 'PECOS QIR Engine with proven quantum operation support' if QIR_ENGINE_AVAILABLE else 'QIR engine not available'
    }