...
class GuppySimulationConfig:
    ...
    quantum_engine_builder: Any | None = None  # Direct engine builder object
    verbose: bool = False
    debug: bool = False
    optimize: bool = True
    binary_string_format: bool = False  # Match qasm_sim option
    keep_intermediate_files: bool = False  # Keep compilation artifacts
    max_qubits: int | None = None  # Maximum number of qubits to simulate (REQUIRED)
...
class GuppySimulation:
    ...
    def __init__(
        ...
        if hasattr(guppy_func, "__name__"):
            ...
            self.function_name = (
                f"guppy_{hashlib.md5(str(guppy_func).encode()).hexdigest()[:8]}"
            )
    ...
    def _convert_noise_model(self, noise_model):
        ...
        type(noise_model).__name__

        if isinstance(noise_model, DepolarizingNoise):
            # Uniform depolarizing noise
            return DepolarizingNoiseModelBuilder().with_uniform_probability(
                noise_model.p,
            )
        if isinstance(noise_model, DepolarizingCustomNoise):
            ...
            return builder.with_p2_probability(noise_model.p2)
        if isinstance(noise_model, BiasedDepolarizingNoise):
            # For biased depolarizing, use uniform probability
            # (BiasedDepolarizingNoiseModelBuilder might need different parameters)
            return BiasedDepolarizingNoiseModelBuilder().with_uniform_probability(
                noise_model.p,
            )
    ...
    def _create_temp_file(self) -> None:
        ...
        if self._config.keep_intermediate_files:
            ...
            fd, path = tempfile.mkstemp(
                suffix=".ll",
                prefix=f"guppy_{self.function_name}_",
            )
        ...
        if self._config.verbose:
            print(
                f"Created {'persistent' if self._config.keep_intermediate_files else 'temporary'} file: {self._temp_file}",
            )

    @classmethod
    def cleanup_all_temp_files(cls) -> None:
    ...
    def run(self, shots: int) -> dict[str, Any]:
        ...
        if RUST_EXECUTION_AVAILABLE and llvm_engine is not None:
            ...
            builder = llvm_engine().program(LlvmProgram.from_string(llvm_ir)).to_sim()
            ...
            if RUST_EXECUTION_AVAILABLE:
                ...
                if result.get("execution_successful", False):
                    ...
                    msg = f"Execution failed: {result.get('error', 'Unknown error')}"
                    raise RuntimeError(
                        msg,
                    )
            else:
                msg = "Rust execution backend not available"
        ...
        if (
            RUST_EXECUTION_AVAILABLE
            and llvm_engine is not None
    ...
    def _run_with_selene_backend(self, shots: int, start_time: float) -> dict[str, Any]:
        ...
        """
        try:
            from pecos.frontends.selene_native_backend import SeleneNativeBackend
        except ImportError:
            if self._config.verbose:
                print(
                    "[WARNING] SeleneNativeBackend not available, falling back to placeholder results",
                )
    ...
    def _format_selene_results(
        self,
        selene_results: list[dict[str, Any]],
    ) -> dict[str, list[Any]]:
    ...
    def _process_llvm_engine_results(
        self,
        ...
        result_keys = [k for k in raw_results if k.startswith("result_")]
        if result_keys:
...
class GuppySimulationBuilder:
    ...
    def __init__(self, guppy_func: Callable, max_qubits: int) -> None:
        ...
        if not GUPPY_AVAILABLE:
            msg = "guppylang is not available. Install with: pip install quantum-pecos[guppy]"
            raise ImportError(
                msg,
            )
        ...
        if not is_guppy:
            ...
            msg = f"Function {func_name} must be decorated with @guppy"
            raise ValueError(msg)

        # Validate max_qubits
        if not isinstance(max_qubits, int) or max_qubits < 1:
            msg = "max_qubits must be a positive integer"
            raise ValueError(msg)
        ...
        self._built = False
        self._simulation: GuppySimulation | None = None
    ...
    def build(self) -> GuppySimulation:
        ...
        try:
            ...
            if "HUGR version incompatibility" in str(e):
                if self._config.verbose:
                    print(
                        "  [WARNING] HUGR version incompatibility detected, using Selene compiler",
                    )

                # Use GuppySeleneCompiler to generate proper LLVM IR
                try:
                    from pecos.frontends.guppy_selene_compiler import (
                        GuppySeleneCompiler,
                    )
                    ...
                    if llvm_file.exists():
                        ...
                        if ll_files:
                            ...
                            msg = "No LLVM IR file generated by Selene compiler"
                            raise RuntimeError(
                                msg,
                            )
                    ...
                    if self._config.verbose:
                        print(
                            f"  Generated LLVM with Selene compiler: {llvm_time:.4f}s ({len(llvm_ir)} bytes)",
                        )
                    ...
                    func_name = getattr(
                        self.guppy_func,
                        "name",
                        getattr(self.guppy_func, "__name__", "quantum_func"),
                    )
...
def guppy_sim(guppy_func: Callable, max_qubits: int) -> GuppySimulationBuilder:
    ...
    Example:
        ...
        >>> results = (
        ...     guppy_sim(bell_state, max_qubits=10).binary_string_format().run(100)
        ... )
