...
class ParallelOptimizer:
    ...
    def _transform_block(self, block: Block) -> Block:
        ...
        if isinstance(block, If):
            ...
            if type(block) == Parallel:
                new_block = Parallel(*new_ops)
            elif type(block) == Main:
    ...
    def _group_operations(self, operations: list[Node], dep_graph: dict[int, set[int]]) -> list[list[Node]]:
        """Group operations by gate type and dependencies.
        ...
        for level in levels:
            ...
            order = ['H', 'X', 'Y', 'Z', 'S', 'T', 'RX', 'RY', 'RZ', 'CX', 'CY', 'CZ', 'Measure', 'Other']
            for op_type in order:
                if op_type in type_groups:
                    grouped.append(type_groups[op_type])
                    
            # Add any remaining types
            for op_type, ops in type_groups.items():
                if op_type not in order:
                    grouped.append(ops)
                    
        return grouped
    
    def _topological_levels(self, n: int, dep_graph: dict[int, set[int]]) -> list[list[int]]:
