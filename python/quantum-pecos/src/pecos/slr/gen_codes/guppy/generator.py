...
class GuppyGenerator(Generator):
    ...
    def get_output(self) -> str:
        ...
        while self.pending_functions:
            ...
            if len(item) == 3:
                ...
                self._generate_function_definition_by_info(func_name, sample_block, block_name)
    ...
    def enter_block(self, block) -> tuple:
        ...
        return (previous_scope, previous_unpacked, previous_measurement_info, previous_consumed)
    ...
    def exit_block(self, previous_state) -> None:
        ...
        if isinstance(previous_state, tuple):
            if len(previous_state) == 4:
                previous_scope, previous_unpacked, previous_measurement_info, previous_consumed = previous_state
                ...
                current_block_type = type(self.current_scope).__name__ if self.current_scope else None
                if current_block_type not in ['If', 'Else', 'Main']:
                    self.consumed_qubits = previous_consumed
            else:
                # Old format
                previous_scope, previous_unpacked, previous_measurement_info = previous_state
                self.current_scope = previous_scope
                self.unpacked_arrays = previous_unpacked
                self.measurement_info = previous_measurement_info
        else:
    ...
    def _generate_function_definition(self, block_type: type, func_name: str, sample_block: Block) -> None:
    ...
    def _analyze_quantum_resource_flow(self, block: Block) -> tuple[dict[str, set[int]], dict[str, set[int]]]:
            ...
            if var_name in self.variable_context:
                ...
                if var_type_name == "QReg":
                    ...
                    if var_name in consumed_qubits or var_name in live_qubits:
                        params.append(f"{var_name}: array[quantum.qubit, {size}] @owned")
                    else:
                    ...
                    for qarg in op.qargs:
                        if hasattr(qarg, 'reg') and hasattr(qarg.reg, 'sym'):
                            ...
                            if reg_name not in param_set:
                                ...
                                params.append(f"{reg_name}: array[quantum.qubit, {size}]")
    ...
    def _generate_function_definition_by_info(self, func_name: str, sample_block: Block, block_name: str) -> None:
        ...
        self.measurement_info = self.measurement_analyzer.analyze_block(sample_block, self.variable_context)
                ...
                if len(info) == 2:
                    ...
                    if qreg_name in self.unpacked_arrays:
                        ...
                        if isinstance(unpacked_names, list):
                            ...
                            live_vars = [unpacked_names[i] for i in sorted_indices if i < len(unpacked_names)]
                            if live_vars:
