...
class QubitUsage:
    ...
    first_use_line: int
    last_use_line: int
    consumption_line: Optional[int] = None
    ...
    def is_consumed_early(self) -> bool:
        ...
        return (self.consumption_line is not None and 
                not self.reused_after_consumption)
...
class AllocationDecision:
    ...
    local_elements: Set[int] = field(default_factory=set)  # Which elements to allocate locally
...
class AllocationOptimizer:
    ...
    def __init__(self):
        self.qubit_usage: Dict[str, Dict[int, QubitUsage]] = {}  # array_name -> index -> usage
        self.current_line = 0
    ...
    def _record_qubit_use(self, qarg, op_type: str) -> None:
        ...
        if array_name:
            if index is not None:
                ...
                if array_name in self.qubit_usage and index in self.qubit_usage[array_name]:
    ...
    def _extract_qubit_ref(self, qarg) -> Tuple[Optional[str], Optional[int]]:
        ...
        elif hasattr(qarg, 'sym') and hasattr(qarg, 'size'):
    ...
    def _make_allocation_decision(self, array_name: str, elements: Dict[int, QubitUsage]) -> AllocationDecision:
        ...
        for index, usage in elements.items():
            if usage.first_use_line == float('inf'):
                ...
                decision.reasons.append(f"Element {index} allocated but not used in operations")
                continue
        ...
        local_candidates = short_lived_elements & early_consumed_elements & single_scope_elements
        ...
        if len(local_candidates) > 0:
            ...
            if len(local_candidates) < len(elements):
                ...
                decision.reasons.append(f"Elements {local_candidates} can be allocated locally")
            else:
                # Full local allocation - disable for now until implementation is complete
                decision.strategy = AllocationStrategy.PRE_ALLOCATE
                decision.reasons.append("All elements are short-lived and consumed early (local allocation not fully implemented)")
    ...
    def generate_optimization_report(self, decisions: Dict[str, AllocationDecision]) -> str:
