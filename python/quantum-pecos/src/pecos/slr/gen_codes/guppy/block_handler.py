...
from .naming import get_function_name
...
class BlockHandler:
    ...
    CORE_BLOCKS = {'If', 'Repeat', 'While', 'Main', 'Block'}
    ...
    def _handle_main_block(self, block) -> None:
        ...
        self.generator.measurement_info = self.generator.measurement_analyzer.analyze_block(block, self.generator.variable_context)
        ...
        if creg_names:
            ...
            for creg_name in creg_names:
                ...
                if hasattr(self.generator, 'renamed_vars') and creg_name in self.generator.renamed_vars:
    ...
    def _handle_if_block(self, block) -> None:
        ...
        from .conditional_handler import ConditionalResourceTracker
    ...
    def _handle_generic_block(self, block) -> None:
        ...
        func_name = self._get_or_create_function_name_by_info(block_key, original_block_name, original_block_module)
        ...
        if block_key not in self.generated_functions:
            self._generate_block_function_by_info(block_key, func_name, block, original_block_name)
    ...
    def _generate_var_declaration(self, var) -> None:
        ...
        if var_type == "QReg":
            ...
            self.generator.write(f"{var_name} = array(quantum.qubit() for _ in range({var.size}))")
        elif var_type == "CReg":
            self.generator.var_types[var_name] = "classical"
            self.generator.write(f"{var_name} = array(False for _ in range({var.size}))")
        else:
            ...
            if hasattr(var, 'vars'):
                ...
                self.generator.write(f"# TODO: Initialize {var_type} instance '{var_name}'")
    ...
    def _generate_block_function(self, block_type: type, func_name: str, sample_block: Block) -> None:
    ...
    def _get_or_create_function_name_by_info(self, block_key: tuple, block_name: str, block_module: str) -> str:
        ...
        if block_key not in self.block_to_function_name:
            ...
            from .naming import class_to_function_name, get_module_prefix
    ...
    def _generate_block_function_by_info(self, block_key: tuple, func_name: str, sample_block: Block, block_name: str) -> None:
        """Generate a function definition using block info."""
        # Add the function to pending functions to be generated later
        self.generator.pending_functions.append((block_key, func_name, sample_block, block_name))
    ...
    def _generate_function_call(self, func_name: str, block: Block) -> None:
        ...
        consumed_qregs, live_qregs = self.generator._analyze_quantum_resource_flow(block)
        ...
        if live_qregs:
            ...
            if len(return_vars) == 1:
                ...
                if return_vars[0].endswith("_remaining"):
                    ...
                    orig_name = return_vars[0].replace("_remaining", "")
    ...
    def _handle_measurement_results(self, block) -> None:
        ...
        individual_measurements = self.generator.operation_handler.individual_measurements
        ...
        for qreg_name, unpacked_info in self.generator.unpacked_arrays.items():
            if isinstance(unpacked_info, str) and unpacked_info.startswith("__measure_array"):
                ...
                if len(measurements) == creg_size:
                    ...
                    for i in range(creg_size):
                        if i in measurements:
                            ...
                            sorted_vars.append(f"False")  # Default value
                    
                    self.generator.write(f"{creg_name} = array({', '.join(sorted_vars)})")
            ...
            for qreg_name, indices in sorted(unconsumed_by_reg.items()):
                ...
                if len(indices) == qreg.size and set(indices) == set(range(qreg.size)):
                        ...
                        if isinstance(unpacked_info, list):
                            ...
                            for i in indices:
                                if i < len(unpacked_info):
                                    self.generator.write(f"_ = quantum.measure({unpacked_info[i]})")
                                else:
                                    self.generator.write(f"_ = quantum.measure({qreg_name}[{i}])")
                    ...
                    for i in indices:
                        if qreg_name in self.generator.unpacked_arrays:
                            unpacked_info = self.generator.unpacked_arrays[qreg_name]
                            if isinstance(unpacked_info, list) and i < len(unpacked_info):
                                self.generator.write(f"_ = quantum.measure({unpacked_info[i]})")
                            else:
                                self.generator.write(f"_ = quantum.measure({qreg_name}[{i}])")
                        else:
