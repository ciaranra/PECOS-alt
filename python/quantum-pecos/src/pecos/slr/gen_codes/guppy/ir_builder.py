...
from .ir import (
    ...
    Assignment, FunctionCall, MethodCall, ArrayAccess, VariableRef,
    Literal, Comment, IfStatement, WhileStatement, ForStatement, ArrayUnpack, Measurement,
    VariableInfo, ScopeContext, ResourceState, BinaryOp, UnaryOp,
    ReturnStatement, TupleExpression, FieldAccess
)
from .ir_analyzer import UnpackingPlan
from .scope_manager import ScopeManager, ScopeType, ResourceUsage
from .allocation_optimizer import AllocationOptimizer, AllocationStrategy
from .qubit_usage_analyzer import QubitUsageAnalyzer, QubitRole
...
class IRBuilder:
    ...
    CORE_BLOCKS = {'If', 'Repeat', 'While', 'For', 'Main', 'Block', 'Comment', 'Barrier'}
    
    def __init__(self, unpacking_plan: UnpackingPlan, include_optimization_report: bool = False):
        ...
        self.discovered_functions = set()  # Functions discovered but maybe not built yet
        ...
        self.struct_info = {}  # Maps prefix -> {fields: [(suffix, type, size)], struct_name: str}
    ...
    def build_module(self, main_block: SLRBlock, pending_functions: List) -> Module:
        ...
        self.allocation_decisions = self.allocation_optimizer.analyze_program(main_block)
        ...
        if self.include_optimization_report and self.allocation_decisions:
            report = self.allocation_optimizer.generate_optimization_report(self.allocation_decisions)
        ...
        while all_pending:
            ...
            if func:
                ...
                for new_func in self.pending_functions:
                    ...
                    already_pending = any(f[1] == new_name for f in all_pending if len(f) >= 2)
    ...
    def build_main_function(self, block: SLRBlock) -> Function:
        ...
        usage_stats = usage_analyzer.analyze_block(block, self.struct_info)
        self.allocation_recommendations = usage_analyzer.get_allocation_recommendations()
        ...
        for array_name in self.plan.unpack_at_start:
            if array_name in self.plan.arrays_to_unpack:
                ...
                # Skip unpacking for arrays that are struct fields (already consumed by struct constructor)
                ...
                if is_struct_field:
                    ...
                    self.current_block.statements.append(
                        Comment(f"Skip unpacking {array_name} - consumed by struct constructor")
                    )
                    continue
    ...
    def build_function(self, func_info) -> Optional[Function]:
        ...
        # Black Box Pattern: All functions that handle quantum arrays should use functional pattern
        # This maintains SLR's global array semantics at boundaries while using functional internals
        ...
        for var in sorted(deps['quantum'] & deps['reads']):
            ...
            is_excluded_ancilla = (hasattr(self, 'ancilla_qubits') and var in self.ancilla_qubits)
        ...
        # Add classical parameters (no ownership, but include written vars since arrays are mutable)
        for var in sorted(deps['classical'] & (deps['reads'] | deps['writes'])):
            ...
            if var_info:
                ...
                param_type = f"array[bool, {var_info.size}]"
            else:
                param_type = "array[bool, 32]"  # Default assumption with size
            params.append((param_name, param_type))
        ...
        has_owned_struct_params = any("@owned" in param_type and param_name in self.struct_info 
        ...
        if needs_unpacking:
            for param_name, param_type in params:
                if "array[quantum.qubit," in param_type and param_name in block_plan.arrays_to_unpack:
                    # Extract array size
                    import re
                    ...
                    if match:
                        ...
                        unpacking_stmt = self._create_array_unpack_statement(param_name, element_names)
        ...
        for param_name, param_type in params:
            ...
            is_ancilla = hasattr(self, 'ancilla_qubits') and param_name in self.ancilla_qubits
            if is_ancilla and "@owned" in param_type and "array[quantum.qubit," in param_type:
                # Check if already unpacked above
                if param_name not in self.unpacked_vars:
                    ...
                    if match:
                        ...
                        body.statements.append(
                            Comment(f"Unpack ancilla array {param_name} to avoid MoveOutOfSubscriptError with @owned")
                        )
        ...
        struct_field_vars = {}  # Maps original var name to struct field path for @owned structs
        struct_reconstruction = {}  # Maps struct param name to list of field vars for reconstruction
        ...
        for param_name, param_type in params:
            if "@owned" in param_type and param_name in self.struct_info:
                ...
                continue
            elif param_name in self.struct_info:
                # Non-owned struct parameter - can unpack normally
                struct_info = self.struct_info[param_name]
                field_vars = []
                
                # Generate unpacking statement - use same order as struct definition (sorted by suffix)
                unpack_targets = []
                for suffix, field_type, field_size in sorted(struct_info['fields']):
                ...
                # Create the unpacking statement: field1, field2, ... = struct.field1, struct.field2, ...
                # In Guppy, we need to unpack the entire struct at once - use same order as struct definition
                unpack_stmt = Assignment(
                    target=TupleExpression([VariableRef(var) for var in unpack_targets]),
                    value=TupleExpression([FieldAccess(VariableRef(param_name), field) 
        ...
        has_quantum_arrays = any("array[quantum.qubit," in ptype for name, ptype in params)
            ...
            if quantum_returns:
                ...
                if len(quantum_returns) == 1:
                    ...
                    if name in consumed_in_function and "array[quantum.qubit," in ptype:
                        ...
                        match = re.search(r'array\[quantum\.qubit, (\d+)\]', ptype)
                        if match:
                            return_size = int(match.group(1))
                            original_match = re.search(r'array\[quantum\.qubit, (\d+)\]', 
                            ...
                            if original_match:
                                ...
                                for i in range(original_size):
                                    if i not in consumed_indices:
                                        if name in self.unpacked_vars:
                                            ...
                                            unconsumed_elements.append(VariableRef(element_name))
                                        else:
                                            # Use array indexing
                                ...
                                body.statements.append(ReturnStatement(value=array_expr))
                        ...
                        body.statements.append(ReturnStatement(value=array_construction))
                        ...
                        # They're only valid if we haven't passed the struct to any @owned functions
                        ...
                        if unpacked_vars_valid:
                            # Create struct constructor call - use same order as struct definition (sorted by suffix)
                            constructor_args = []
                            for suffix, field_type, field_size in sorted(struct_info['fields']):
                    ...
                    if return_exprs:
                        body.statements.append(ReturnStatement(value=TupleExpression(elements=return_exprs)))
    ...
    def _add_variable_declaration(self, var, block=None) -> None:
        ...
        if var_type == "QReg":
            ...
            # But only if it doesn't need unpacking for selective measurements AND not used in full array ops
            if recommendation.get('allocation') == 'dynamic' and not needs_unpacking and not needs_full_array:
                ...
                if is_function_param:
                    ...
                    self.current_block.statements.append(
                        Comment(f"Pre-allocate ancilla array {var_name} (used as function parameter)")
                    )
                    ...
                    self.current_block.statements.append(
                        Comment(f"# {var_name} will be allocated dynamically ({recommendation.get('reason', 'ancilla pattern')})")
                    )
                ...
                if needs_unpacking:
                    ...
                    self.current_block.statements.append(assignment)
                    self.current_block.statements.append(
                        Comment(f"Note: Full pre-allocation used because {var_name} needs unpacking")
                    )
                elif decision.original_size - len(decision.local_elements) > 0:
                    pre_alloc_size = decision.original_size - len(decision.local_elements)
                    init_expr = FunctionCall(
                        func_name="array",
                        args=[
                            FunctionCall(
                ...
                self.current_block.statements.append(
                    Comment(f"Elements {sorted(decision.local_elements)} of {var_name} will be allocated locally")
                )
            else:
    ...
    def _block_consumes_quantum(self, block) -> bool:
        ...
        if self._block_accesses_struct_quantum_fields(block):
            return True
    ...
    def _block_accesses_struct_quantum_fields(self, block) -> bool:
        ...
        if self.struct_info:
            ...
            for op in block.ops:
                ...
                if op_name in ['H', 'X', 'Y', 'Z', 'CX', 'CY', 'CZ', 'Reset', 'Measure', 'S', 'T', 'Sdg', 'Tdg']:
                    return True
                
                # Also check for nested quantum operations
                if hasattr(op, 'ops'):
                    if self._block_accesses_struct_quantum_fields(op):
                        return True
    ...
    def _needs_unpacking_workaround(self, block) -> bool:
        ...
        for op in block.ops:
            ...
            if hasattr(op, 'ops'):
                if self._needs_unpacking_workaround(op):
                    return True
    ...
    def _create_array_unpack_statement(self, array_name: str, element_names: List[str]) -> Statement:
    ...
    def _create_struct_construction(self, struct_name: str, field_names: List[str], field_values: List[Expression]) -> Expression:
    ...
    def _convert_operation(self, op) -> Optional[Statement]:
    ...
    def _convert_measurement(self, meas) -> Optional[Statement]:
            ...
            if hasattr(qarg, 'reg') and hasattr(qarg.reg, 'sym'):
                ...
                for prefix, info in self.struct_info.items():
                    if array_name in info['var_names'].values():
                        # Check if we're in a function with @owned struct
                        if hasattr(self, 'function_info') and hasattr(self, 'current_function_name'):
                            ...
                            if func_info.get('has_owned_struct_params', False):
                                ...
                                self.current_block.statements.append(
                                    Comment("WARNING: Measuring qubits from @owned struct arrays is not supported by guppylang")
                                )
                                self.current_block.statements.append(
                                    Comment("This will cause a MoveOutOfSubscriptError during compilation")
                                )
        ...
        if (len(meas.qargs) == 1 and hasattr(meas.qargs[0], 'sym') and 
            hasattr(meas.qargs[0], 'size') and meas.qargs[0].size >= 1):
            ...
            self.scope_manager.track_resource_usage(qreg.sym, set(range(qreg.size)), consumed=True)
            
            # Check if this array was dynamically allocated
            if hasattr(self, 'dynamic_allocations') and qreg.sym in self.dynamic_allocations:
        ...
        if len(meas.qargs) == 1:
            ...
            if hasattr(qarg, 'reg') and hasattr(qarg.reg, 'sym') and hasattr(qarg, 'index'):
                ...
                self.scope_manager.track_resource_usage(array_name, {qubit_index}, consumed=True)
            ...
            is_main = hasattr(self, 'current_function_name') and self.current_function_name == 'main'
            if (not is_main and hasattr(self, 'unpacked_vars') and hasattr(qarg, 'reg') and 
                hasattr(qarg.reg, 'sym') and hasattr(qarg, 'index')):
                array_name = qarg.reg.sym
                qubit_index = qarg.index
    ...
    def _convert_qubit_ref(self, qarg) -> IRNode:
        ...
        if hasattr(qarg, 'reg') and hasattr(qarg.reg, 'sym'):
            ...
            if hasattr(self, 'struct_field_mapping') and original_array in self.struct_field_mapping:
                ...
                if '.' in struct_field_path:
                    ...
                    if hasattr(qarg, 'index'):
                        ...
                        field_access = FieldAccess(obj=VariableRef(struct_name), field=field_name)
            ...
            if hasattr(self, 'dynamic_allocations') and original_array in self.dynamic_allocations:
                # For dynamic allocations, we allocate individual qubits as needed
                if hasattr(qarg, 'index'):
            ...
            if hasattr(self, 'var_remapping') and original_array in self.var_remapping:
                ...
                if hasattr(qarg, 'index'):
                    ...
                    return ArrayAccess(array=VariableRef(unpacked_var_name), index=qarg.index)
                else:
            ...
            for prefix, info in self.struct_info.items():
                if array_name in info['var_names'].values():
                    ...
                    suffix = [k for k, v in info['var_names'].items() if v == array_name][0]
                    ...
                    if hasattr(self, 'param_mapping'):
                        # We're in a function - check if the struct is a parameter
                        if prefix in self.param_mapping:
                            struct_param_name = self.param_mapping[prefix]
                    
                    if hasattr(qarg, 'index'):
                        # Struct field element access: c.d[0]
                        field_access = FieldAccess(obj=VariableRef(struct_param_name), field=suffix)
                        return ArrayAccess(array=field_access, index=qarg.index)
                    else:
            ...
            if hasattr(qarg, 'index'):
                ...
                for check_name in check_names:
                    if hasattr(self, 'unpacked_vars') and check_name in self.unpacked_vars:
    ...
    def _convert_bit_ref(self, carg, is_assignment_target=False) -> IRNode:
        ...
        if hasattr(carg, 'reg') and hasattr(carg.reg, 'sym'):
            ...
            if hasattr(self, 'var_remapping') and original_array in self.var_remapping:
                ...
                if hasattr(carg, 'index'):
                    ...
                    return ArrayAccess(array=VariableRef(unpacked_var_name), index=carg.index)
                else:
            ...
            if hasattr(carg, 'index'):
                ...
                var_info = self.context.lookup_variable(array_name)
                if (not is_assignment_target and var_info and var_info.is_unpacked 
    ...
    def _convert_quantum_gate(self, gate) -> Optional[Statement]:
        ...
        args = []
        expanded_single_qubit_gate = False
        if hasattr(gate, 'qargs') and gate.qargs:
            # Check if this is a single-qubit gate with multiple arguments
            if gate_name in ["H", "X", "Y", "Z", "S", "SZ", "SZdg", "T", "Tdg", "Prep"] and len(gate.qargs) > 1:
                # Single-qubit gate applied to multiple qubits
                # Check if all qargs are consecutive array elements from the same array
                if (all(hasattr(qarg, 'reg') and hasattr(qarg, 'index') for qarg in gate.qargs) and
                    len(set(qarg.reg.sym for qarg in gate.qargs)) == 1):
            ...
            # Handle multi-qubit gates with tuple arguments
            elif gate_name in ["CX", "CY", "CZ"] and all(isinstance(arg, tuple) and len(arg) == 2 for arg in gate.qargs):
                ...
                for qarg in gate.qargs:
                    ...
                    if hasattr(qarg, 'sym') and hasattr(qarg, 'size') and qarg.size > 1:
                        ...
                        # However, reset operations in functions with owned arrays need special handling
                        
                        if (gate_name == "Prep" and hasattr(self, 'var_remapping') and 
                            self.var_remapping and array_name in self.var_remapping):
                            # Array Unpacking Pattern: use unpacked variables with functional operations
                            ...
                            if hasattr(self, 'unpacked_vars') and array_name in self.unpacked_vars:
                                ...
                                for i in range(min(qarg.size, len(element_names))):
                                    ...
                                    call = FunctionCall(func_name=func_name, args=[elem_var])
                            ...
                            # The key insight is that if we're in a function with @owned struct parameters
                            ...
                            if hasattr(self, 'var_remapping') and array_name in self.var_remapping:
                                ...
                                for prefix, info in self.struct_info.items():
                                    if array_name in info['var_names'].values() and hasattr(self, 'current_function_params'):
                                        # Check if the struct is an @owned parameter
                                        for param_name, param_type in self.current_function_params:
                                            if param_name == prefix and '@owned' in str(param_type):
                                                use_unpacked = True
                                                break
                                        if use_unpacked:
                                            break
                            ...
                            if use_unpacked:
                                    ...
                                    remapped_name = self.var_remapping.get(array_name, array_name)
                                    elem_ref = ArrayAccess(array=VariableRef(remapped_name), index=VariableRef(loop_var))
                                    call = FunctionCall(func_name=func_name, args=[elem_ref])
                                    ...
                                    if qarg.sym in info['var_names'].values():
                                        ...
                                        for suffix, var_name in info['var_names'].items():
                                            if var_name == qarg.sym:
                                                ...
                                                field_access = FieldAccess(obj=VariableRef(struct_param_name), field=suffix)
                                                elem_ref = ArrayAccess(array=field_access, index=VariableRef(loop_var))
                                                call = FunctionCall(func_name=func_name, args=[elem_ref])
                            ...
                            if not is_struct_field:
                                ...
                                elem_ref = ArrayAccess(array=VariableRef(actual_array_name), index=VariableRef(loop_var))
                                call = FunctionCall(func_name=func_name, args=[elem_ref])
    ...
    def _convert_if(self, if_block) -> Optional[Statement]:
        """Convert If block."""
        # Check if this If block has struct field access in loop with @owned parameters
        if hasattr(if_block, 'cond') and self._is_struct_field_in_loop_with_owned(if_block.cond):
            ...
            if condition_var:
                self.current_block.statements.append(
                    Comment(f"Extract condition variable to avoid @owned struct field access in loop")
                )
                ...
                self.current_block.statements.append(
                    Comment("Fallback: If condition with struct field access simplified for @owned compatibility")
                )
        ...
        for res_name, indices in self.consumed_resources.items():
            consumed_before_if[res_name] = indices.copy() if isinstance(indices, set) else set(indices)
        ...
        unbalanced = self.scope_manager.analyze_conditional_branches(then_scope, else_scope_info, self.context)
        ...
        if unbalanced:
            ...
            def add_resource_consumption(block, res_name, indices):
                ...
                if indices:
                    block.statements.append(Comment("Consume qubits to maintain linearity"))
            ...
            if else_block:
                ...
                for res_name, indices in unbalanced.items():
                    if res_name in then_scope.resource_usage:
                        ...
                        else_usage = else_scope_info.resource_usage.get(res_name, ResourceUsage(res_name, set()))
                        missing_in_then = else_usage.consumed - then_usage.consumed
                        if missing_in_then:
                            add_resource_consumption(then_block, res_name, missing_in_then)
                ...
                else_block = Block()
                else_block.statements.append(Comment("Auto-generated else block for linearity"))
    ...
    def _convert_while(self, while_block) -> Optional[Statement]:
        """Convert While loop."""
        # Convert condition
        ...
        with self.scope_manager.enter_scope(ScopeType.LOOP) as loop_scope:
    ...
    def _convert_for(self, for_block) -> Optional[Statement]:
        """Convert For loop."""
        ...
        elif hasattr(for_block, 'start') and hasattr(for_block, 'stop'):
    ...
    def _convert_for_range(self, for_block, loop_var) -> Optional[Statement]:
    ...
    def _convert_for_iterable(self, for_block, loop_var) -> Optional[Statement]:
    ...
    def _convert_condition(self, cond) -> IRNode:
        ...
        if cond_type == "Bit":
            ...
            if (isinstance(right, Literal) and right.value == 1 and 
                type(cond.left).__name__ == "Bit"):
        ...
        elif hasattr(cond, 'value'):
            # Literal value
            return Literal(cond.value)
        elif isinstance(cond, (int, bool, str)):
            # Direct literal
            return Literal(cond)
    ...
    def _convert_repeat(self, repeat_block) -> Optional[Statement]:
    ...
    def _convert_comment(self, comment) -> Optional[Statement]:
        """Convert comment."""
    ...
    def _extract_condition_variable(self, cond) -> Optional[dict]:
        ...
        if cond_type == "EQUIV" and hasattr(cond, 'left') and hasattr(cond, 'right'):
            ...
            if (hasattr(left, 'reg') and hasattr(left.reg, 'sym') and 
                hasattr(left, 'index')):
                ...
                for prefix, info in self.struct_info.items():
                    if array_name in info['var_names'].values():
                        ...
                        if field_name:
                            ...
                            compare_value = getattr(right, 'val', right) if hasattr(right, 'val') else right
    ...
    def _convert_set_operation(self, set_op) -> Optional[Statement]:
    ...
    def _convert_permute(self, permute) -> Optional[Statement]:
        ...
        if hasattr(permute, 'elems_i') and hasattr(permute, 'elems_f'):
            ...
            # Case 2: List of elements permutation
                ...
                return self._generate_permutation_code(permutation_map, elems_i, elems_f)
    ...
    def _elements_equal(self, elem1, elem2):
        ...
        if (hasattr(elem1, 'reg') and hasattr(elem1, 'index') and 
            hasattr(elem2, 'reg') and hasattr(elem2, 'index')):
            return (elem1.reg.sym == elem2.reg.sym and 
                    elem1.index == elem2.index)
        # Both are full register references
        elif hasattr(elem1, 'sym') and hasattr(elem2, 'sym'):
    ...
    def _find_permutation_cycles(self, permutation_map):
        ...
        for start in permutation_map:
            ...
            if len(cycle) > 0 and (len(cycle) > 1 or cycle[0] != permutation_map.get(cycle[0], cycle[0])):
                cycles.append(cycle)
    ...
    def _convert_block_call(self, block) -> Optional[Statement]:
        ...
        if block_signature not in self.block_registry:
            ...
            for prefix, info in self.struct_info.items():
                ...
                # If this block operates on variables from this struct, use QEC code name if available
            ...
            if original_block_module and original_block_module != '__main__':
                ...
                if module_name and module_name.startswith('test_'):
                    ...
                    func_name = self._generate_function_name(f"{module_name}_{base_name}", struct_prefix)
                else:
    ...
    def _generate_function_name(self, block_name: str, struct_prefix: Optional[str] = None) -> str:
        ...
        if struct_prefix:
            base_name = f"{struct_prefix}_{snake_case}"
        else:
    ...
    def _generate_function_call(self, func_name: str, block) -> Statement:
        ...
        quantum_args = [arg for arg in quantum_args if isinstance(arg, str)]  # Filter for array names
        
        # Check if we're returning structs
        returning_structs = any(arg in self.struct_info for arg in quantum_args)
        ...
        if quantum_args and (not function_consumes or function_returns_something):
            ...
            if len(quantum_args) == 1:
                ...
                if name in self.struct_info and hasattr(self, 'var_remapping'):
                    struct_info = self.struct_info[name]
                    # Check if any of the struct's fields are in var_remapping (indicating unpacking)
                    needs_re_unpack = any(
                    ...
                    if needs_re_unpack:
                        ...
                        # consumed by the function call. Instead, we need to update our var_remapping
                        ...
                        statements.append(Comment(
                            f"Note: Cannot use unpacked variables after calling function with @owned struct"
                        ))
                        
                        # Update var_remapping to indicate these variables should not be used
                        # by mapping them back to struct field access
                        for suffix, var_name in struct_info['var_names'].items():
                ...
                targets = [name for name in quantum_args]
                ...
                for array_name in quantum_args:
                    if array_name in self.struct_info and hasattr(self, 'var_remapping'):
                ...
                for array_name in quantum_args:
                    if array_name in self.plan.unpack_at_start and array_name not in self.struct_info:
                        # Get the array info to determine size
                        if array_name in self.plan.arrays_to_unpack:
                            info = self.plan.arrays_to_unpack[array_name]
                            self._add_array_unpacking(array_name, info.size)
        ...
        if self.struct_info:
            for prefix, info in self.struct_info.items():
                ...
                if func_name.startswith(f"{base_name}_"):
                    ...
                    # Exception: functions ending in 'discard' or 'decompose' don't return the struct
                    if func_name.endswith('_discard') or func_name.endswith('_decompose'):
                        return False
                    return True
    ...
    def _analyze_op_dependencies(self, op, deps: Dict[str, set], depth: int = 0) -> None:
    ...
    def _add_final_handling(self, block) -> None:
        ...
        for prefix, info in self.struct_info.items():
            ...
            if has_unconsumed_quantum:
                ...
                func_name = f"{qec_code_name}_decompose" if qec_code_name else f"{prefix}_decompose"
                ...
                # Create assignment
                target_tuple = TupleExpression(elements=[VariableRef(name) for name in targets])
                ...
                struct_decompositions[prefix] = list(zip(
                            ...
                            for suffix, mapped_var in info['var_names'].items():
                                if mapped_var == var_name:
                                    ...
                                    if prefix in struct_decompositions:
                                        ...
                                        for field_suffix, decomposed_var, _, _ in struct_decompositions[prefix]:
    ...
    def _add_cleanup(self, block, cleaned_up_arrays=None) -> None:
        ...
        for prefix, info in self.struct_info.items():
            ...
            if needs_cleanup and prefix not in struct_cleanup_done:
                ...
                self.current_block.statements.append(
                    Comment(f"Note: struct {prefix} contains unconsumed quantum arrays")
                )
    ...
    def _check_has_element_operations(self, block, var_name: str) -> bool:
        ...
        for op in block.ops:
            ...
            if hasattr(op, 'qargs'):
                for qarg in op.qargs:
                    if (hasattr(qarg, 'reg') and hasattr(qarg.reg, 'sym') and 
                        qarg.reg.sym == var_name and hasattr(qarg, 'index')):
                        # Operation on array element
                        if op_type in ["Prep", "Measure"]:
                            return True
                            
            # Recursively check nested blocks
            if hasattr(op, 'ops'):
                if self._check_has_element_operations(op, var_name):
                    return True
    ...
    def _track_consumed_qubits(self, op, consumed: Dict[str, Set[int]]) -> None:
            ...
            if hasattr(op, 'qargs') and op.qargs:
                for qarg in op.qargs:
                    ...
                    if hasattr(qarg, 'sym') and hasattr(qarg, 'size'):
                        ...
                        self.scope_manager.track_resource_usage(qreg_name, indices, consumed=True)
                    # Handle individual qubit measurement
                        ...
                        if hasattr(qarg, 'index'):
                            ...
                            self.scope_manager.track_resource_usage(
                                qreg_name, {qarg.index}, consumed=True
                            )
        ...
        # Check else blocks
        if op_type == "If" and hasattr(op, 'else_block') and op.else_block:
    ...
    def _array_needs_full_allocation(self, array_name: str, block) -> bool:
        ...
        for op in block.ops:
            ...
            # Check else blocks
            if (hasattr(op, 'else_block') and op.else_block and 
                hasattr(op.else_block, 'ops')):
                for nested_op in op.else_block.ops:
                    if self._operation_uses_full_array(nested_op, array_name):
                        return True
    ...
    def _operation_uses_full_array(self, op, array_name: str) -> bool:
        ...
        if hasattr(op, 'qargs') and len(op.qargs) == 1:
            ...
            if (hasattr(qarg, 'sym') and qarg.sym == array_name and 
                hasattr(qarg, 'size') and qarg.size > 1 and
                not hasattr(qarg, 'index')):
                return True
        return False
        ...
        if not qec_code_name:
            ...
            def find_qec_code_in_block(op, depth=0, max_depth=5):
                ...
                if hasattr(op, '__class__') and hasattr(op.__class__, '__module__'):
                    ...
                    # Extract QEC code name from module path like 'pecos.qeclib.steane.preps.pauli_states'
            ...
            if len(vars_list) >= 2:
                ...
                if has_quantum:
                    ...
                    self.struct_info[prefix] = {
                        ...
                        'var_names': {v[0]: v[3] for v in vars_list},  # suffix -> full var name
                        'qec_code_name': qec_code_name,  # Store for function naming
                        'ancilla_vars': getattr(self, 'ancilla_qubits', set())  # Track which vars were excluded
                    }
    ...
    def _generate_struct_decompose_function(self, prefix: str, info: dict) -> Optional[Function]:
        ...
        for suffix, var_type, size in sorted(info['fields']):
            ...
            if size > 1:
                return_types.append(f"array[{var_type}, {size}]")
            else:
                return_types.append(var_type)
        ...
        field_refs = [FieldAccess(obj=VariableRef(prefix), field=suffix) 
    ...
    def _generate_struct_discard_function(self, prefix: str, info: dict) -> Optional[Function]:
    ...
    def _add_struct_initialization(self, prefix: str, info: dict, block: SLRBlock) -> None:
        ...
        struct_expr = self._create_struct_construction(struct_name, 
        ...
        self.context.add_variable(VariableInfo(
            name=prefix,
