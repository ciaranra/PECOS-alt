...
if TYPE_CHECKING:
    from pecos.slr import Block, QReg, CReg
...
class MeasurementInfo:
    ...
    def are_measurements_consecutive(self, ops_list) -> bool:
        ...
        for i in range(first_meas, len(ops_list)):
            ...
            if self._is_operation_on_qreg_static(op, self.qreg_name):
                if not self._is_measurement_static(op):
                    return False
    ...
    def _is_measurement_static(op) -> bool:
        ...
        return op_type == "Measure" or (hasattr(op, 'is_measurement') and op.is_measurement)
...
class MeasurementAnalyzer:
    ...
    def analyze_block(self, block: Block, variable_context: Dict[str, Any] = None) -> Dict[str, MeasurementInfo]:
        ...
        for info in self.qreg_info.values():
            if info.is_fully_measured():
                info.all_measured_together = info.are_measurements_consecutive(block.ops)
                # Debug output
                # print(f"DEBUG: {info.qreg_name} all_measured_together={info.all_measured_together}, measured_indices={info.measured_indices}, positions={info.measurement_positions}")
    ...
    def _analyze_operation(self, op, position: int) -> None:
        ...
        if op_type == "If" and hasattr(op, 'else_block') and op.else_block and hasattr(op.else_block, 'ops'):
