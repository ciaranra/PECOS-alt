...
class QubitUsageStats:
    ...
    # Lifetime tracking
    first_use_position: Optional[int] = None
    last_use_position: Optional[int] = None
...
class QubitUsageAnalyzer:
    ...
    def analyze_block(self, block: SLRBlock, struct_info: Dict[str, Dict] = None) -> Dict[str, QubitUsageStats]:
    ...
    def _get_register_name(self, qarg) -> Optional[str]:
    ...
    def get_allocation_recommendations(self) -> Dict[str, Dict]:
        ...
        for reg_name, stats in self.register_stats.items():
            ...
            if role == QubitRole.ANCILLA:
                ...
                recommendations[reg_name] = {
                    'allocation': 'static',
                    'reason': 'Long-lived data qubits' if stats.is_struct_member else f'Low measure/reset ratio ({stats.measure_reset_ratio:.2f})',
                    'keep_packed': True,
                    'pre_allocate': True
                }
            else:
