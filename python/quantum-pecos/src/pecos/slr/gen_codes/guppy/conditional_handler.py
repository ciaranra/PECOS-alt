...
class ConditionalResourceTracker:
    ...
    def analyze_if_block_resources(self, if_block: Block) -> Tuple[Dict[str, Set[int]], Dict[str, Set[int]], Dict[str, Set[int]]]:
        ...
        if hasattr(if_block, 'else_block') and if_block.else_block:
            else_consumed, else_used = self._analyze_branch_resources(if_block.else_block)
        
        # Combine all used resources
        all_used = {}
        for qreg_name in set(then_used.keys()) | set(else_used.keys()):
            all_used[qreg_name] = then_used.get(qreg_name, set()) | else_used.get(qreg_name, set())
            
        return then_consumed, else_consumed, all_used
    
    def _analyze_branch_resources(self, block: Block) -> Tuple[Dict[str, Set[int]], Dict[str, Set[int]]]:
    ...
    def _analyze_op_resources(self, op, consumed_qubits: Dict[str, Set[int]], used_qubits: Dict[str, Set[int]]) -> None:
        ...
        if op_type == "If":
            ...
            if hasattr(op, 'else_block') and op.else_block and hasattr(op.else_block, 'ops'):
    ...
    def generate_resource_cleanup(self, missing_consumed: Dict[str, Set[int]]) -> bool:
        ...
        for qreg_name in sorted(actually_missing.keys()):
            ...
            if qreg and hasattr(qreg, 'size'):
                # If all indices need to be consumed, use measure_array for efficiency
                        ...
                        if isinstance(unpacked_info, list):
                            ...
                            for idx in indices:
                                if idx < len(unpacked_info):
                                    self.generator.write(f"_ = quantum.measure({unpacked_info[idx]})")
                                else:
    ...
    def ensure_branches_consume_same_resources(self, if_block: Block) -> None:
        ...
        then_consumed, else_consumed, all_used = self.analyze_if_block_resources(if_block)
