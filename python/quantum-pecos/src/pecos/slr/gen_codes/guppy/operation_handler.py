...
class OperationHandler:
    ...
    def generate_op(self, op, position: int = -1) -> None:
        ...
        try:
            ...
            self.generator.write(f"# ERROR generating {type(op).__name__}: {e!s}")
            import traceback
    ...
    def _get_qubit_ref(self, qubit) -> str:
        ...
        if (
            ...
            if (
                ...
                and qreg_name in self.generator.renamed_vars
            ):
        ...
        if hasattr(qubit, "reg") and hasattr(qubit, "index"):
            ...
            if (
                hasattr(self.generator, "renamed_vars")
                and reg_name in self.generator.renamed_vars
            ):
        ...
        if hasattr(qubit, "sym"):
            ...
            if (
                hasattr(self.generator, "renamed_vars")
                and var_name in self.generator.renamed_vars
            ):
    ...
    def _check_and_unpack_arrays(self, meas, position: int) -> None:
        ...
        (
            type(self.generator.current_scope).__name__
            if self.generator.current_scope
            else None
        )
        ...
        for qreg_name in qregs_in_measurement:
            if qreg_name in self.generator.measurement_info:
                ...
                if (
                    position == info.first_measurement_pos
                    and info.all_measured_together
                    and qreg_name not in self.generator.unpacked_arrays
                ):
                ...
                if (
                    position == info.first_measurement_pos
                    and not info.all_measured_together
                    and qreg_name not in self.generator.unpacked_arrays
                ):

                    # Generate unpacking code
                    unpacked_names = (
                        self.generator.measurement_analyzer.get_unpacked_var_names(
                            qreg_name,
                            info.qreg_size,
                        )
                    )
    ...
    def _should_use_measure_array(self, meas, position: int) -> tuple[bool, str, str]:
        ...
        if (
            hasattr(meas, "qargs")
            and len(meas.qargs) == 1
            and hasattr(meas.qargs[0], "reg")
        ):
            qarg = meas.qargs[0]
            if hasattr(qarg.reg, "sym"):
                ...
                if (
                    qreg_name in self.generator.measurement_info
                    and self.generator.measurement_info[qreg_name].all_measured_together
                    and position
                    == self.generator.measurement_info[qreg_name].first_measurement_pos
                ):
    ...
    def _generate_measurement(self, meas, position: int = -1) -> None:
        ...
        should_use_array, qreg_name, temp_var_name = self._should_use_measure_array(
            meas,
            position,
        )
        if should_use_array:
            ...
            qreg.size if qreg and hasattr(qreg, "size") else 0

            # Generate measure_array to temporary variable
            self.generator.write(
                f"{temp_var_name} = quantum.measure_array({qreg_name})",
            )
        ...
        if (
            ...
            if hasattr(qarg.reg, "sym") and hasattr(qarg, "index"):
                ...
                if qreg_name in self.generator.unpacked_arrays:
                    ...
                    if (
                        isinstance(unpacked_value, dict)
                        and unpacked_value.get("type") == "measure_array_temp"
                    ):
                        # Handle distribution from temporary array
                        self._handle_measure_array_distribution(meas, qreg_name)
                        return
                    if isinstance(unpacked_value, str) and unpacked_value.startswith(
                        "__measure_array_handled_",
                    ):
        ...
        if hasattr(meas, "cout") and meas.cout:
            ...
            if (
                ...
                and len(meas.cout) == 1
                and hasattr(meas.cout[0], "size")
                and meas.qargs[0].size == meas.cout[0].size
            ):
                ...
                if (
                    qreg.sym in self.generator.measurement_info
                    and self.generator.measurement_info[qreg.sym].all_measured_together
                ):
            ...
            if (
                ...
                and len(meas.cout) == 1
                and hasattr(meas.cout[0], "size")
                and meas.cout[0].size == len(meas.qargs)
            ):
                # Multiple qubits to single register
                creg = meas.cout[0]
                [self._get_qubit_ref(q) for q in meas.qargs]
                self.generator.write(
                    f"# Measure {len(meas.qargs)} qubits to {creg.sym}",
                )
                for i, q in enumerate(meas.qargs):
                    qubit_ref = self._get_qubit_ref(q)
                    self.generator.write(
                        f"{creg.sym}[{i}] = quantum.measure({qubit_ref})",
                    )
                return
    ...
    def _generate_barrier(self, op) -> None:
        ...
        _ = op  # Barrier operations don't need op details
    ...
    def _generate_prep(self, op) -> None:
        ...
        if hasattr(op, "qargs") and op.qargs:
            ...
            if (
                len(op.qargs) == 1
                and hasattr(op.qargs[0], "size")
                and op.qargs[0].size > 1
            ):
                # Full register reset
                reg = op.qargs[0]
    ...
    def _generate_permute(self, op) -> None:
        ...
        if len(op.qargs) == 2:
            ...
            if hasattr(qreg1, "sym") and hasattr(qreg2, "sym"):
                ...
                self.generator.write("# TODO: Implement register swap")
            else:
                self.generator.write("# WARNING: Permute with non-register arguments")
    ...
    def _generate_individual_measurement(
        self,
        q,
        c,
        qubit_ref: str,
        ...
        in_main = (
            self.generator.current_scope
            and type(self.generator.current_scope).__name__ == "Main"
        )

        # Check if this is measuring an unpacked qubit IN MAIN FUNCTION with valid classical register
        if (
            in_main
            and hasattr(q, "reg")
