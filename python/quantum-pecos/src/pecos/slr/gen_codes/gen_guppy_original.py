...
class GuppyGenerator(Generator):
    ...
    def _handle_generic_block(self, block) -> None:
        ...
        if hasattr(block, "ops"):
            ...
            self.write(
                f"# TODO: Handle {block_name} block - no specific handler implemented",
            )
    ...
    def generate_op(self, op) -> None:
        ...
        try:
            ...
            # Handle bitwise operations
                ...
                self.write(
                    f"# Module: {op.__module__ if hasattr(op, '__module__') else 'unknown'}",
                )
                self.write(
                    f"# Attributes: {[attr for attr in dir(op) if not attr.startswith('_')][:5]}...",
            ...
            self.write(f"# Exception: {type(e).__name__}: {e!s}")
    ...
    def _generate_barrier(self, op) -> None:
        ...
        _ = op  # Unused - barriers don't have a direct equivalent in Guppy
        ...
        if hasattr(op, "elems_i") and hasattr(op, "elems_f"):
            ...
            if hasattr(elems_i, "sym") and hasattr(elems_f, "sym"):
                ...
                if hasattr(elems_i, "size") and hasattr(elems_f, "size"):
                    if elems_i.size == elems_f.size:
                        ...
                        self.write(
                            f"# Permute registers {elems_i.sym} <-> {elems_f.sym}",
                        )
                        self.write(f"for i in range({elems_i.size}):")
                        self.indent()
                        self.write(
                            f"{elems_i.sym}[i], {elems_f.sym}[i] = {elems_f.sym}[i], {elems_i.sym}[i]",
                        )
                        self.dedent()
                    else:
                        self.write(
                            f"# ERROR: Cannot permute registers of different sizes "
                            f"({elems_i.sym}: {elems_i.size}, {elems_f.sym}: {elems_f.size})",
                        )
                else:
                    # Simple variable swap
                    self.write(
                        f"{elems_i.sym}, {elems_f.sym} = {elems_f.sym}, {elems_i.sym}",
                    )

            # Handle single element permutation (e.g., Permute(q[0], q[1]))
            elif (
                hasattr(elems_i, "reg")
                and hasattr(elems_i, "index")
                and hasattr(elems_f, "reg")
                and hasattr(elems_f, "index")
            ):
                # Single qubit/bit swap
                ref_i = self._get_qubit_ref(elems_i)
                ref_f = self._get_qubit_ref(elems_f)
                self.write("# Permute single elements")
            ...
            # Handle element-level permutation
                ...
                if len(elems_i) == len(elems_f):
                    ...
                    self.write("# Permute elements")
                    self.write(f"{left_side} = {right_side}")
                else:
                    self.write(
                        f"# ERROR: Permute lists must have same length (got {len(elems_i)} and {len(elems_f)})",
                    )
            else:
                self.write("# WARNING: Permute operation with unexpected structure")
                self.write(
                    f"# elems_i type: {type(elems_i)}, elems_f type: {type(elems_f)}",
                )
        else:
            self.write(
                "# ERROR: Permute operation missing required attributes (elems_i, elems_f)",
            )
    ...
    def _generate_bitwise_expr(self, expr, parent_op=None) -> str:
        ...
        if op_name == "XOR":
            ...
            if (
                hasattr(expr.value, "__class__")
                and type(expr.value).__name__ in precedence
            ):
                result = f"not ({value})"
            else:
                result = f"not {value}"
        ...
        if (
            parent_op
            and op_name in precedence
            and parent_op in precedence
            and precedence[op_name] < precedence[parent_op]
        ):
            result = f"({result})"
