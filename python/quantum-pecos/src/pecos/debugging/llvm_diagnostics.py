...
try:
    from pecos_rslib import get_qir_diagnostic_report, validate_qir_format_detailed
...
class QirDiagnostics:
    ...
    def validate_qir_file(qir_path: str) -> dict[str, Any]:
        ...
        if not _DIAGNOSTICS_AVAILABLE:
            msg = "QIR diagnostics not available - pecos_rslib not installed"
            raise ImportError(
                msg,
            )

        if not os.path.exists(qir_path):
            msg = f"QIR file not found: {qir_path}"
    ...
    def analyze_qir_content(qir_content: str) -> dict[str, Any]:
        ...
        has_opaque_types = (
            "%Result = type opaque" in qir_content
            or "%Qubit = type opaque" in qir_content
        )
        uses_i64_ops = "__quantum__qis__h__body(i64" in qir_content
        uses_ptr_ops = (
            "__quantum__qis__h__body(i8*" in qir_content
            or "__quantum__qis__h__body(%Qubit*" in qir_content
        )
        ...
        if has_opaque_types and uses_ptr_ops:
            ...
            analysis["issues"].append(
                "Mixed calling conventions detected - both i64 and pointer types",
            )
    ...
    def diagnose_execution_error(
        ...
        if "index out of bounds" in error_lower:
            ...
            diagnosis["likely_cause"] = (
                "Qubit or result index exceeds allocated resources"
            )
        ...
        if qir_path and os.path.exists(qir_path):
            try:
                with open(qir_path) as f:
                    qir_content = f.read()
                diagnosis["qir_analysis"] = QirDiagnostics.analyze_qir_content(
                    qir_content,
                )
            except Exception as e:
                diagnosis["qir_analysis"] = {
                    "error": f"Failed to analyze QIR file: {e}",
                }
    ...
    def print_diagnosis(diagnosis: dict[str, Any]) -> None:
        ...
        if diagnosis.get("qir_analysis"):
            ...
            if analysis.get("statistics"):
                ...
                print(
                    f"  Statistics: {stats.get('quantum_operations', 0)} quantum ops, "
...
def validate_qir(qir_path: str, verbose: bool = True) -> bool:
    ...
    try:
        ...
        if verbose:
            ...
            print("📊 Statistics:")
            print(
                f"   Format Type: {'Standard QIR' if stats['has_opaque_types'] else 'HUGR-style'}",
            )
...
def diagnose_error(error_message: str, qir_path: str | None = None) -> None:
