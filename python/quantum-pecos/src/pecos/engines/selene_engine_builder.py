...
try:
    ...
    from selene_sim.backends import SimpleRuntime, IdealErrorModel
...
try:
    ...
    class RustSeleneEngine:
        def __init__(self, *args, **kwargs):
            raise ImportError("SeleneLibraryEngine not available in pecos_rslib")
...
class SeleneEngineBuilder:
    ...
    def with_guppy_program(self, program: Any) -> "SeleneEngineBuilder":
        ...
        if not GUPPY_AVAILABLE:
            raise ImportError("guppylang is required for Guppy programs")
    ...
    def with_hugr_program(self, program: Any) -> "SeleneEngineBuilder":
        ...
        if hasattr(program, 'inner'):
            ...
            raise ValueError(f"Unsupported HUGR program type: {type(program)}")
    ...
    def _compile_to_hugr(self) -> bytes:
        ...
        if self.guppy_program is None:
            raise ValueError("No Guppy program provided")
            
        if not COMPILATION_AVAILABLE:
            raise ImportError("Compilation pipeline not available")
    ...
    def _build_shared_library(self, hugr_bytes: bytes) -> Path:
        ...
        if not SELENE_AVAILABLE:
            raise ImportError("selene_sim is required for building")
        ...
        try:
            ...
            if executable.exists():
                logger.warning("Selene built executable instead of library, attempting conversion...")
                return self._convert_executable_to_library(executable)
                
            raise RuntimeError("No library found after build")
            
        except Exception as e:
            raise RuntimeError(f"Failed to build Selene library: {e}")
