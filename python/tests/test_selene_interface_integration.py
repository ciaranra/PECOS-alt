...
def test_selene_interface_program_available():
...
def test_selene_compilation_pipeline():
    ...
    from guppylang.prelude.quantum import Qubit, H, Measure
...
def test_sim_with_selene_interface():
    ...
    try:
        ...
        try:
            ...
            assert "runtime" in str(e).lower() or "library" in str(e).lower() or "load" in str(e).lower()
...
def test_runtime_library_finding():
    ...
    possible_paths = [
        Path("/home/ciaranra/Repos/cl_projects/gup/PECOS/lib/pecos-runtimes/libselene_simple_runtime.so"),
    ...
    if venv:
        ...
        for version in ["python3.12", "python3.11", "python3.10"]:
            runtime_path = venv_path / f"lib/{version}/site-packages/selene_simple_runtime_plugin/_dist/lib/libselene_simple_runtime.so"
    ...
    for path in possible_paths:
        if path.exists():
            ...
            with open(path, 'rb') as f:
                header = f.read(4)
                if header == b'\x7fELF':
                    print(f"  ✓ Valid ELF shared library")
                else:
                    print(f"  ✗ Not a valid ELF file")
    
    if not found_any:
        print("No Selene runtime libraries found (this is OK if Selene is not installed)")
