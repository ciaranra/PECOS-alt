...
import pytest
from pecos_rslib import sim, HugrProgram, selene_engine
...
def test_selene_hugr_llvm_generation() -> None:
    ...
    try:
        ...
        engine = selene_engine().program(hugr_prog)
        ...
        assert "not yet implemented" in str(e).lower() or "processing error" in str(e).lower()
...
def test_bell_state_llvm_ir_generation() -> None:
    ...
    try:
        from pecos_rslib._pecos_rslib import generate_bell_state_llvm, generate_quantum_llvm_ir
    except ImportError:
