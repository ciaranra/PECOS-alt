...
def test_guppy_phir_execute_pipeline() -> None:
    ...
    try:
        phir_mlir = hugr_to_phir_mlir(
            hugr_json,
            debug_output=False,
            optimization_level=2,
        )
        print("[PASS] HUGR → PHIR conversion successful")
    ...
    try:
        ...
        print("[PASS] PHIR → LLVM IR compilation successful")
    ...
    try:
        ...
        assert (
            "define i32 @main()" in llvm_content or "define i1 @main()" in llvm_content
        )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print("  [PASS] PECOS CLI execution successful!")
                ...
                if lines:
                    ...
                    if measurements:
                        ...
                        print(
                            f"  [PASS] Executed {len(measurements)} shots successfully!",
                        )
                        print(
                            f"    True results: {true_count} ({true_count/len(measurements)*100:.1f}%)",
                        )
                        print(
                            f"    False results: {false_count} ({false_count/len(measurements)*100:.1f}%)",
                        )

                        # For a Hadamard + measure, we expect roughly 50/50 distribution
                        if (
                            15 <= true_count <= 35
                        ):  # Allow for quantum randomness (30-70% range)
                            print("  [PASS] Results show expected quantum randomness!")
                        else:
                            print(
                                f"  [INFO] Results: {true_count}/{len(measurements)} - may still be valid quantum behavior",
                            )
                        ...
                        execution_successful = (
                            True  # CLI worked even if we couldn't parse results
                        )
                ...
                print(
                    f"  [ERROR] PECOS CLI failed with return code {result.returncode}",
                )
        ...
        if not execution_successful:
            try:
                ...
                if result.get("execution_successful"):
                    print("  [PASS] PhirLlvmEngine execution successful!")
                    print(f"    Results: {result}")
                    execution_successful = True
                    results = result
                else:
                    print(
                        f"  [ERROR] PhirLlvmEngine failed: {result.get('error', 'Unknown error')}",
                    )
        ...
        if execution_successful:
            ...
            print("  The generated LLVM IR is valid and ready for execution")
            print("  Install PECOS quantum runtime to enable execution")
