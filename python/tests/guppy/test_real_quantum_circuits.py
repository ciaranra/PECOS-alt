...
from guppylang.std.quantum import h, cx, measure, qubit, x, z, ry, rz
...
def test_bell_state_preparation():
    ...
    # Run simulation
    results = sim(prepare_bell_state) \
        .qubits(2) \
        .seed(42) \
        .run(1000)
...
def test_quantum_phase_kickback():
    ...
    # Run simulation
    results = sim(phase_kickback_circuit) \
        .qubits(2) \
        .seed(42) \
        .run(1000)
    ...
    assert control_one_count / total > 0.9, f"Control should be ~100% |1⟩ after phase kickback, got {control_one_count / total}"
    # Target should remain |1⟩
    assert target_one_count / total > 0.9, f"Target should remain |1⟩, got {target_one_count / total}"
    
    print(f"Phase kickback test passed: control |1⟩={control_one_count}/{total}, target |1⟩={target_one_count}/{total}")
...
def test_quantum_interference():
    ...
    # Run simulation
    results = sim(quantum_interferometer) \
        .qubits(1) \
        .seed(42) \
        .run(1000)
    ...
    if isinstance(results, dict):
        for outcome, count in results.items():
            total += count
            if outcome == True or outcome == 1 or outcome == "1":
                one_count += count
...
def test_rotation_gates():
    ...
    if isinstance(results, dict):
        for outcome, count in results.items():
            if outcome == False or outcome == 0 or outcome == "0":
                zero_count += count
            else:
                one_count += count
