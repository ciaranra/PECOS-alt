...
class TestQubitAllocationLimits:
    ...
    def test_dynamic_allocation_in_loop(self):
        ...
        results = sim(dynamic_loop_test).qubits(10).quantum(state_vector()).seed(42).run(10)
        ...
        assert all(0 <= v <= 3 for v in results.get("measurements", results.get("measurement_1", [])))
    ...
    def test_dynamic_allocation_exceeds_limit(self):
        ...
        with pytest.raises(RuntimeError) as exc_info:
            results = sim(exceed_limit_test).qubits(3).quantum(state_vector()).run(1)
    ...
    def test_nested_loop_allocation(self):
        ...
        results = sim(nested_loop_test).qubits(10).quantum(state_vector()).seed(42).run(50)
    ...
    def test_allocation_with_reset(self):
        ...
        results = sim(reset_reuse_test).qubits(10).quantum(state_vector()).seed(42).run(50)
