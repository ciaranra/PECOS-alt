...
def test_raw_results() -> None:
    ...
    if "_result" in results:
        ...
        print(
            f"_result length: {len(results.get("measurements", results.get("measurement_1", [])))}",
        )
        if len(results.get("measurements", results.get("measurement_1", []))) > 0:
            print(
                f"First shot type: {type(results.get("measurements", results.get("measurement_1", []))[0])}",
            )
            print(
                f"First shot value: {results.get("measurements", results.get("measurement_1", []))[0]}",
            )

            # Decode if tuple
            if isinstance(
                results.get("measurements", results.get("measurement_1", []))[0],
                tuple,
            ):
                r1, r2, r3, r4 = results.get(
                    "measurements",
                    results.get("measurement_1", []),
                )[0]
                print("\nDecoded first shot:")
