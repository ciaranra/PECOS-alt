#!/usr/bin/env python3
"""Test the complete Guppy → HUGR → PMIR → LLVM → execution pipeline.

This demonstrates using actual Guppy code compiled via the PMIR alternative path.
"""
...
def test_guppy_pmir_execute_pipeline() -> None:
    """Test the full Guppy → PMIR → execution pipeline."""
    print("Testing Guppy → HUGR → PMIR → LLVM → execution")
    print("=" * 60)
    
    # Test 1: Check if PMIR is available
    print("\n1. Checking PMIR availability...")
    try:
        ...
        assert PMIR_AVAILABLE
        print("[PASS] PMIR module loaded successfully")
    except (ImportError, AssertionError) as e:
        print(f"[SKIP] PMIR not available: {e}")
        import pytest
        pytest.skip("PMIR functionality not available")
    ...
    try:
        ...
        print(f"  PMIR size: {len(pmir_mlir)} characters")
        print("\nGenerated PMIR (first 300 chars):")
        print(pmir_mlir[:300] + "..." if len(pmir_mlir) > 300 else pmir_mlir)
    except Exception as e:
    ...
    try:
        ...
        print(f"[PASS] PMIR → LLVM IR compilation successful")
    ...
    print("executable LLVM IR via the PMIR alternative path. The generated LLVM IR")
...
if __name__ == "__main__":
    # Run the test
    try:
        test_guppy_pmir_execute_pipeline()
    except Exception as e:
