...
def test_python_guppy_to_plugin():
...
def test_python_guppy_hugr_to_selene():
...
def test_python_llvm_to_selene():
...
def test_full_guppy_to_selene_execution():
    """Test: Full Guppy → Selene → Execution (setup only)."""
    try:
        from guppylang import guppy
        from guppylang.std.quantum import h, measure, qubit
        from pecos_rslib.sim import selene_engine, state_vector, sim
        from pecos_rslib.programs import HugrProgram
    except ImportError as e:
        pytest.skip(f"Required imports not available: {e}")
