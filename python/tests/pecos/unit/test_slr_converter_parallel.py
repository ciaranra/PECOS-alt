...
def test_slr_converter_without_optimization():
...
def test_slr_converter_optimization_preserves_semantics():
    ...
    for gate in ['h q[0]', 'h q[2]', 'cx q[0], q[1]', 'cx q[2], q[3]', 'measure q -> m']:
    ...
    assert qasm_opt.index('h q[0]') < qasm_opt.index('cx q[2], q[3]')  # Cross-pair ordering


def test_slr_converter_no_parallel_blocks():
